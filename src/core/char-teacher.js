'use strict';
/**
 * 汉字教学内容生成器：读字、组词、解释、造句、用法。
 *
 * 数据源：
 * - 组词：从 words.json 中提取包含该字的常用词（优先短词、高频词）
 * - 造句：从 sentences.json 中提取包含该字的儿童句子
 * - 解释/用法：首版用简化模板；预留 AI 接口（setAITeacher），
 *   未来接入大模型后可生成准确的字义解释和用法说明。
 */
const fs = require('fs');
const path = require('path');

let _words = null;
let _sentences = null;
let _sentencesByChar = null;
let _xinhua = null;
let _dataDir = null;

// AI 教学内容生成器接口（预留）
// interface AITeacher { generate(hanzi, context): Promise<{meaning, usage}> }
let aiTeacher = null;

function init(dataDir) {
  _dataDir = dataDir;
}

function loadXinhua() {
  if (_xinhua) return _xinhua;
  if (!_dataDir) return {};
  try {
    _xinhua = JSON.parse(fs.readFileSync(path.join(_dataDir, 'xinhua.json'), 'utf8'));
  } catch (e) {
    _xinhua = {};
  }
  return _xinhua;
}

function loadWords() {
  if (_words) return _words;
  if (!_dataDir) return [];
  try {
    _words = JSON.parse(fs.readFileSync(path.join(_dataDir, 'words.json'), 'utf8'));
  } catch (e) {
    _words = [];
  }
  return _words;
}

function loadSentences() {
  if (_sentences) return _sentences;
  if (!_dataDir) return [];
  try {
    _sentences = JSON.parse(fs.readFileSync(path.join(_dataDir, 'sentences.json'), 'utf8'));
  } catch (e) {
    _sentences = [];
  }
  return _sentences;
}

function loadSentencesByChar() {
  if (_sentencesByChar) return _sentencesByChar;
  if (!_dataDir) return {};
  try {
    _sentencesByChar = JSON.parse(fs.readFileSync(path.join(_dataDir, 'sentences-by-char.json'), 'utf8'));
  } catch (e) {
    _sentencesByChar = {};
  }
  return _sentencesByChar;
}

/**
 * 组词：从词库中提取包含该字的常用词，优先短词（2-3字）。
 * 只返回词语（word/phrase），不返回句子（sentence）。
 * 词库中找不到时，用内置常见词表兜底。
 * @param {string} hanzi
 * @param {number} count
 * @returns {Array<{word:string,type:string}>}
 */
function getWords(hanzi, count = 3) {
  // 优先使用内置常见词（更适合儿童）
  const fallback = FALLBACK_WORDS[hanzi] || [];
  if (fallback.length > 0) {
    return fallback.slice(0, count).map((w) => ({ word: w, type: 'word' }));
  }

  const words = loadWords();
  const candidates = words
    .filter((w) => w.chars && w.chars.includes(hanzi))
    .filter((w) => w.type === 'word' || w.type === 'phrase') // 只返回词语，不返回句子
    .sort((a, b) => a.chars.length - b.chars.length); // 短词优先

  if (candidates.length > 0) {
    return candidates.slice(0, count).map((w) => ({ word: w.text, type: w.type }));
  }

  return [];
}

// 内置常见词表（词库中缺少的常见词，特别是包含生僻字的词）
const FALLBACK_WORDS = {
  '柠': ['柠檬'],
  '檬': ['柠檬'],
  '螃': ['螃蟹'],
  '蟹': ['螃蟹', '河蟹', '蟹黄'],
  '蝴': ['蝴蝶'],
  '蝶': ['蝴蝶', '蝶泳', '蝴蝶结'],
  '蜻': ['蜻蜓'],
  '蜓': ['蜻蜓'],
  '蚂': ['蚂蚁', '蚂蚱'],
  '蚁': ['蚂蚁', '白蚁', '蚁穴'],
  '葡': ['葡萄', '葡萄酒', '葡萄糖'],
  '萄': ['葡萄', '葡萄干'],
  '咖': ['咖啡', '咖喱'],
  '啡': ['咖啡'],
  '巧': ['巧克力', '巧妙', '巧合'],
  '克': ['巧克力', '克服', '千克'],
  '力': ['巧克力', '力量', '努力'],
  '沙': ['沙发', '沙子', '沙滩'],
  '发': ['沙发', '发现', '头发'],
  '吉': ['吉他', '吉祥', '吉利'],
  '他': ['吉他', '他们', '其他'],
  '目': ['目光', '目录', '耳目'],
  '耳': ['耳朵', '耳机', '木耳'],
  '鼻': ['鼻子', '鼻涕', '鼻梁'],
  '嘴': ['嘴巴', '嘴唇', '嘴角'],
  '舌': ['舌头', '舌尖', '舌苔'],
  '牙': ['牙齿', '牙膏', '牙刷'],
  '头': ['头发', '头脑', '点头'],
  '脸': ['脸上', '脸盆', '笑脸'],
  '手': ['手里', '手指', '小手'],
  '脚': ['脚步', '脚印', '山脚'],
  '腿': ['大腿', '小腿', '腿脚'],
  '臂': ['手臂', '臂力', '双臂'],
  '背': ['背书', '背包', '后背'],
  '胸': ['胸口', '胸膛', '挺胸'],
  '腹': ['腹部', '腹肌', '小腹'],
  '腰': ['腰带', '弯腰', '山腰'],
  '肩': ['肩膀', '肩负', '双肩'],
  '颈': ['颈部', '长颈鹿', '瓶颈'],
  '皮': ['皮肤', '皮球', '树皮'],
  '骨': ['骨头', '骨折', '骨骼'],
  '血': ['血液', '血压', '鲜血'],
  '肉': ['肉类', '猪肉', '果肉'],
  '筋': ['筋骨', '橡皮筋', '脑筋'],
  '脉': ['脉搏', '山脉', '叶脉'],
  '汗': ['汗水', '出汗', '汗流浃背'],
  '泪': ['眼泪', '泪水', '流泪'],
  '心': ['心里', '心脏', '开心'],
  '肝': ['肝脏', '猪肝', '心肝'],
  '肺': ['肺脏', '肺炎', '心肺'],
  '胃': ['胃口', '胃痛', '肠胃'],
  '肠': ['肠道', '香肠', '大肠'],
  '脑': ['大脑', '脑筋', '电脑'],
  '眼': ['眼睛', '眼泪', '双眼'],
  '口': ['口渴', '门口', '开口'],
  '一': ['一个', '一起', '第一'],
  '二': ['二月', '第二', '二手'],
  '三': ['三个', '三月', '第三'],
  '四': ['四个', '四季', '第四'],
  '五': ['五个', '五月', '第五'],
  '六': ['六个', '六月', '第六'],
  '七': ['七个', '七月', '第七'],
  '八': ['八个', '八月', '第八'],
  '九': ['九个', '九月', '第九'],
  '十': ['十个', '十月', '第十'],
  '人': ['人们', '人人', '大人'],
  '大': ['大象', '大家', '高大'],
  '小': ['小猫', '小鸟', '大小'],
  '上': ['上面', '上学', '天上'],
  '下': ['下面', '下雨', '地下'],
  '左': ['左边', '左手', '左右'],
  '右': ['右边', '右手', '左右'],
  '中': ['中间', '中国', '心中'],
  '里': ['里面', '家里', '心里'],
  '外': ['外面', '外国', '门外'],
  '前': ['前面', '前进', '以前'],
  '后': ['后面', '后来', '以后'],
  '东': ['东方', '东边', '东西'],
  '西': ['西边', '西瓜', '东西'],
  '南': ['南方', '南瓜', '南北'],
  '北': ['北方', '北京', '南北'],
  '天': ['天空', '今天', '天气'],
  '地': ['地上', '地球', '土地'],
  '日': ['日出', '生日', '日子'],
  '月': ['月亮', '月饼', '月光'],
  '水': ['水里', '喝水', '河水'],
  '火': ['火车', '大火', '火光'],
  '山': ['山上', '爬山', '高山'],
  '石': ['石头', '石桥', '岩石'],
  '田': ['田里', '田野', '稻田'],
  '土': ['土里', '土地', '泥土'],
  '木': ['木头', '树木', '木板'],
  '禾': ['禾苗', '禾田', '禾穗'],
  '米': ['米饭', '大米', '玉米'],
  '车': ['车子', '坐车', '汽车'],
  '马': ['马儿', '马路', '骑马'],
  '牛': ['牛奶', '小牛', '黄牛'],
  '羊': ['羊毛', '小羊', '山羊'],
  '鸟': ['鸟儿', '小鸟', '飞鸟'],
  '鱼': ['鱼儿', '钓鱼', '金鱼'],
  '虫': ['虫子', '昆虫', '小虫'],
  '花': ['花儿', '花园', '鲜花'],
  '草': ['草地', '小草', '青草'],
  '树': ['树上', '树林', '大树'],
  '林': ['森林', '竹林', '树林'],
  '果': ['果园', '苹果', '水果'],
  '瓜': ['西瓜', '南瓜', '瓜子'],
  '豆': ['豆子', '豆腐', '黄豆'],
  '麦': ['麦田', '麦子', '小麦'],
  '粮': ['粮食', '粮仓', '口粮'],
  '油': ['汽油', '香油', '加油'],
  '盐': ['盐水', '海盐', '食盐'],
  '醋': ['米醋', '吃醋', '香醋'],
  '茶': ['茶水', '喝茶', '茶叶'],
  '酒': ['酒店', '喝酒', '白酒'],
  '饭': ['饭菜', '吃饭', '米饭'],
  '菜': ['菜园', '蔬菜', '白菜'],
  '蛋': ['鸡蛋', '蛋糕', '鸭蛋'],
  '奶': ['牛奶', '奶奶', '奶粉'],
  '糖': ['糖果', '冰糖', '白糖'],
  '饼': ['饼干', '月饼', '大饼'],
  '面': ['面条', '面包', '面粉'],
  '包': ['书包', '包子', '面包'],
  '饺': ['饺子', '水饺', '蒸饺'],
  '汤': ['汤水', '喝汤', '米汤'],
  '粥': ['喝粥', '八宝粥', '米粥'],
  '糕': ['蛋糕', '年糕', '糕点'],
  '点': ['点心', '点头', '雨点'],
  '宜': ['适宜', '便宜', '宜人'],
  '鹤': ['仙鹤', '白鹤', '丹顶鹤'],
  '嫌': ['嫌弃', '嫌疑', '讨嫌'],
  '朱': ['朱红', '朱砂', '朱门'],
  '嵌': ['镶嵌', '嵌入', '嵌进'],
  '匣': ['匣子', '木匣', '镜匣'],
  '嗜': ['嗜好', '嗜睡', '嗜学'],
  '哨': ['哨兵', '口哨', '岗哨'],
  '恩': ['恩情', '感恩', '恩人'],
  '韵': ['韵味', '音韵', '风韵'],
  '亩': ['一亩', '亩产', '田亩'],
  '播': ['播种', '广播', '播放'],
  '浇': ['浇水', '浇灌', '浇花'],
  '吩': ['吩咐'],
  '咐': ['吩咐', '嘱咐', '托咐'],
  '榨': ['榨油', '压榨', '榨取'],
  '慕': ['羡慕', '爱慕', '仰慕'],
  '箩': ['箩筐', '淘箩', '笸箩'],
  '浸': ['浸泡', '浸湿', '沉浸'],
  '缠': ['缠绕', '纠缠', '缠人'],
  '捡': ['捡起', '捡拾', '捡漏'],
  '汛': ['汛期', '防汛', '潮汛'],
  '访': ['访问', '拜访', '采访'],
  '挽': ['挽回', '挽救', '挽留'],
  '隔': ['隔开', '隔离', '相隔'],
  '懒': ['懒惰', '懒散', '偷懒'],
  '惰': ['懒惰', '惰性', '怠惰'],
  '衡': ['平衡', '衡量', '均衡'],
  '协': ['协调', '协助', '合作'],
  '绰': ['绰号', '阔绰', '绰绰有余'],
  '召': ['召开', '召唤', '召集'],
  '臣': ['大臣', '忠臣', '臣民'],
  '缺': ['缺少', '缺点', '缺口'],
  '献': ['贡献', '奉献', '献血'],
  '诺': ['诺言', '承诺', '许诺'],
  '典': ['字典', '典礼', '经典'],
  '抄': ['抄写', '抄袭', '抄书'],
  '拒': ['拒绝', '抗拒', '拒收'],
  '荆': ['荆棘', '紫荆', '负荆请罪'],
  '冠': ['冠军', '皇冠', '鸡冠'],
  '俯': ['俯视', '俯身', '俯瞰'],
  '枚': ['一枚', '枚举', '不胜枚举'],
  '箭': ['火箭', '射箭', '弓箭'],
  '筒': ['竹筒', '笔筒', '邮筒'],
  '束': ['结束', '束缚', '一束'],
  '赤': ['赤红', '赤脚', '赤子'],
  '置': ['放置', '位置', '布置'],
  '侵': ['侵略', '侵犯', '入侵'],
  '筑': ['建筑', '修筑', '筑路'],
  '堡': ['城堡', '堡垒', '碉堡'],
  '党': ['党员', '政党', '入党'],
  '丘': ['山丘', '丘陵', '沙丘'],
  '妨': ['妨碍', '妨害', '不妨'],
  '蔽': ['遮蔽', '隐蔽', '蒙蔽'],
  '陷': ['陷阱', '陷入', '缺陷'],
  '拐': ['拐弯', '拐杖', '拐卖'],
  '酬': ['报酬', '应酬', '酬谢'],
  '珍': ['珍贵', '珍惜', '珍珠'],
  '叮': ['叮嘱', '叮咬', '叮当'],
  '嘱': ['嘱咐', '叮嘱', '遗嘱'],
  '塌': ['倒塌', '塌陷', '塌方'],
  '焦': ['焦急', '烧焦', '焦点'],
  '誓': ['誓言', '发誓', '宣誓'],
  '谎': ['谎话', '撒谎', '谎言'],
  '延': ['延长', '延伸', '拖延'],
  '悔': ['后悔', '悔恨', '懊悔'],
  '扶': ['扶手', '搀扶', '扶起'],
  '郎': ['新郎', '郎中', '儿郎'],
  '爹': ['爹爹', '老爹', '爹娘'],
  '嫂': ['嫂子', '大嫂', '嫂嫂'],
  '辆': ['车辆', '一辆', '几辆'],
  '歹': ['歹徒', '好歹', '歹毒'],
  '纱': ['纱布', '纱窗', '棉纱'],
  '妻': ['妻子', '夫妻', '妻儿'],
  '托': ['托付', '委托', '托起'],
  '溜': ['溜走', '溜冰', '溜达'],
  '婚': ['结婚', '婚姻', '婚礼'],
  '辈': ['长辈', '辈分', '前辈'],
  '挨': ['挨打', '挨饿', '挨着'],
  '祭': ['祭祀', '祭奠', '祭坛'],
  '乃': ['乃至', '乃是', '乃尔'],
  '熏': ['熏陶', '熏肉', '熏黑'],
  '杭': ['杭州', '苏杭', '杭绸'],
  '亥': ['亥时', '己亥', '辛亥'],
  '恃': ['恃才', '有恃无恐', '自恃'],
  '拘': ['拘束', '拘留', '拘谨'],
  '泻': ['腹泻', '倾泻', '泻药'],
  '潜': ['潜水', '潜伏', '潜力'],
  '胎': ['胎儿', '轮胎', '胎记'],
  '皇': ['皇帝', '皇宫', '皇冠'],
  '履': ['履行', '履历', '步履'],
  '疆': ['边疆', '疆域', '新疆'],
  '毁': ['毁灭', '毁坏', '烧毁'],
  '估': ['估计', '估算', '评估'],
  '拱': ['拱桥', '拱手', '拱门'],
  '辉': ['辉煌', '光辉', '余晖'],
  '煌': ['辉煌', '敦煌', '煌煌'],
  '陵': ['陵墓', '丘陵', '中山陵'],
  '境': ['环境', '境界', '边境'],
  '唐': ['唐朝', '唐代', '唐诗'],
  '闯': ['闯荡', '闯关', '闯祸'],
  '统': ['统一', '统治', '系统'],
  '销': ['销售', '销毁', '推销'],
  '奉': ['奉献', '奉命', '奉承'],
  '摄': ['摄影', '拍摄', '摄氏'],
  '氏': ['姓氏', '氏族', '摄氏'],
  '殖': ['繁殖', '殖民', '养殖'],
  '炭': ['煤炭', '木炭', '炭火'],
  '菌': ['细菌', '病菌', '真菌'],
  '疗': ['治疗', '疗养', '医疗'],
  '鼠': ['老鼠', '鼠标', '松鼠'],
  '玲': ['玲珑', '玲玲', '小巧玲珑'],
  '珑': ['玲珑', '珑璁', '小巧玲珑'],
  '歇': ['歇息', '停歇', '歇脚'],
  '窝': ['鸟窝', '窝藏', '酒窝'],
  '拾': ['拾起', '捡拾', '收拾'],
  '狭': ['狭窄', '狭小', '狭隘'],
  '勉': ['勉强', '勉励', '勤勉'],
  '梳': ['梳子', '梳头', '梳理'],
  '辞': ['辞职', '推辞', '修辞'],
  '抑': ['抑制', '压抑', '抑郁'],
  '碌': ['忙碌', '庸碌', '劳碌'],
  '吊': ['吊车', '吊灯', '吊桥'],
  '酷': ['酷热', '残酷', '酷爱'],
  '暑': ['暑假', '酷暑', '中暑'],
  '噪': ['噪音', '噪声', '聒噪'],
  '脊': ['脊梁', '屋脊', '脊柱'],
  '罩': ['口罩', '笼罩', '灯罩'],
  '竟': ['竟然', '究竟', '毕竟'],
  '哇': ['哇啦', '哇哇', '好哇'],
  '忍': ['忍受', '忍耐', '残忍'],
  '械': ['机械', '器械', '武器'],
  '酸': ['酸甜', '心酸', '盐酸'],
  '权': ['权利', '权力', '权威'],
  '蚕': ['蚕丝', '蚕茧', '养蚕'],
  '屑': ['纸屑', '不屑', '木屑'],
  '迪': ['启迪', '迪拜', '迪斯科'],
  '钉': ['钉子', '铁钉', '钉住'],
  '陪': ['陪伴', '陪同', '陪读'],
  '毕': ['毕业', '完毕', '毕竟'],
  '煮': ['煮饭', '煮面', '烹煮'],
  '枕': ['枕头', '枕巾', '抱枕'],
  '孙': ['孙子', '孙女', '子孙'],
  '泊': ['停泊', '漂泊', '湖泊'],
  '畔': ['湖畔', '河畔', '耳畔'],
  '黎': ['黎明', '黎族', '巴黎'],
  '晕': ['头晕', '红晕', '晕车'],
  '幕': ['幕布', '夜幕', '屏幕'],
  '愈': ['愈合', '愈加', '痊愈'],
  '旷': ['空旷', '旷课', '心旷神怡'],
  '怡': ['怡然', '怡人', '心旷神怡'],
  '逸': ['安逸', '逃逸', '飘逸'],
  '免': ['免费', '免除', '避免'],
  '桨': ['船桨', '划桨', '木桨'],
  '榕': ['榕树', '榕城', '小叶榕'],
  '纠': ['纠正', '纠纷', '纠缠'],
  '耀': ['照耀', '耀眼', '荣耀'],
  '涨': ['涨价', '涨水', '上涨'],
  '梢': ['树梢', '眉梢', '末梢'],
  '抛': ['抛弃', '抛球', '抛售'],
  '耻': ['耻辱', '羞耻', '可耻'],
  '诲': ['教诲', '训诲', '诲人不倦'],
  '谓': ['所谓', '称谓', '谓语'],
  '诵': ['背诵', '朗诵', '诵读'],
  '岂': ['岂敢', '岂能', '岂有此理'],
  '津': ['天津', '津贴', '津津有味'],
  '斩': ['斩断', '斩首', '斩钉截铁'],
  '凯': ['凯旋', '凯歌', '凯撒'],
  '葛': ['葛根', '瓜葛', '诸葛亮'],
  '贾': ['商贾', '贾宝玉', '贾岛'],
  '衰': ['衰老', '衰退', '兴衰'],
  '刊': ['刊物', '刊登', '报刊'],
  '琐': ['琐碎', '繁琐', '琐屑'],
  '朴': ['朴素', '朴实', '淳朴'],
  '某': ['某个', '某些', '某人'],
  '昼': ['昼夜', '白昼', '昼伏夜出'],
  '耘': ['耕耘', '耘田', '春耕夏耘'],
  '桑': ['桑树', '桑叶', '蚕桑'],
  '晓': ['晓得', '拂晓', '知晓'],
  '蚱': ['蚱蜢', '蚂蚱', '蚱蝉'],
  '嗡': ['嗡嗡', '嗡鸣', '嗡嗡响'],
  '樱': ['樱花', '樱桃', '樱桃树'],
  '榆': ['榆树', '榆钱', '桑榆'],
  '拔': ['拔草', '拔河', '挺拔'],
  '瞎': ['瞎子', '瞎说', '瞎眼'],
  '锄': ['锄头', '锄草', '锄地'],
  '割': ['割草', '收割', '分割'],
  '尾': ['尾巴', '结尾', '末尾'],
  '拴': ['拴住', '拴马', '拴绳'],
  '瓢': ['水瓢', '瓢虫', '瓢泼'],
  '逛': ['逛街', '闲逛', '逛公园'],
  '妒': ['嫉妒', '妒忌', '妒火'],
  '忌': ['忌讳', '嫉妒', '顾忌'],
  '曹': ['曹操', '曹植', '阴曹'],
  '督': ['督促', '监督', '总督'],
  '委': ['委托', '委员', '委屈'],
  '鲁': ['鲁莽', '粗鲁', '山东鲁'],
  '遮': ['遮挡', '遮盖', '遮蔽'],
  '寨': ['山寨', '村寨', '营寨'],
  '擂': ['擂鼓', '擂台', '打擂'],
  '呐': ['呐喊', '唢呐', '摇旗呐喊'],
  '插': ['插入', '插花', '穿插'],
  '冈': ['山冈', '景阳冈', '冈峦'],
  '饥': ['饥饿', '饥荒', '充饥'],
  '碟': ['碟子', '碗碟', '飞碟'],
  '斤': ['斤两', '公斤', '千斤'],
  '俺': ['俺们', '俺家', '俺的'],
  '榜': ['榜样', '榜单', '光荣榜'],
  '杖': ['拐杖', '手杖', '擀面杖'],
  '申': ['申请', '申诉', '申明'],
  '兼': ['兼顾', '兼职', '德才兼备'],
  '勿': ['切勿', '勿扰', '请勿'],
  '拖': ['拖拉', '拖延', '拖把'],
  '悉': ['熟悉', '悉心', '获悉'],
  '坠': ['坠落', '坠毁', '下坠'],
  '膛': ['胸膛', '炉膛', '枪膛'],
  '截': ['截断', '截止', '拦截'],
  '篱': ['篱笆', '篱墙', '竹篱'],
  '仞': ['万仞', '千仞', '壁立千仞'],
  '岳': ['山岳', '岳父', '五岳'],
  '摩': ['摩擦', '按摩', '摩天'],
  '巫': ['巫师', '女巫', '巫术'],
  '彭': ['彭湃', '彭德怀', '姓彭'],
  '拟': ['拟人', '模拟', '拟定'],
  '瑞': ['瑞士', '祥瑞', '瑞雪'],
  '损': ['损失', '损坏', '破损'],
  '锻': ['锻炼', '锻造', '锻铁'],
  '炼': ['锻炼', '冶炼', '磨炼'],
  '眷': ['眷恋', '家眷', '眷属'],
  '赴': ['奔赴', '赴约', '赴汤蹈火'],
  '搞': ['搞定', '搞笑', '搞活动'],
  '殊': ['特殊', '殊荣', '悬殊'],
  '尊': ['尊重', '尊敬', '尊严'],
  '签': ['签字', '签名', '书签'],
  '革': ['革命', '改革', '皮革'],
  '庆': ['庆祝', '国庆', '喜庆'],
  '诊': ['诊断', '门诊', '诊疗'],
  '沃': ['肥沃', '沃土', '沃野'],
  '龄': ['年龄', '工龄', '高龄'],
  '匪': ['土匪', '匪徒', '盗匪'],
  '绷': ['绷带', '绷紧', '紧绷'],
  '审': ['审查', '审核', '审判'],
  '剂': ['药剂', '剂量', '冲剂'],
  '施': ['施工', '实施', '施舍'],
  '吭': ['吭声', '吭气', '一声不吭'],
  '崭': ['崭新', '崭露头角', '崭然'],
  '衷': ['衷心', '苦衷', '由衷'],
  '慈': ['慈祥', '仁慈', '慈爱'],
  '祥': ['吉祥', '祥和', '慈祥'],
  '跤': ['摔跤', '跌跤', '摔跤手'],
  '搂': ['搂住', '搂抱', '搂钱'],
  '仗': ['打仗', '依仗', '对仗'],
  '鞭': ['鞭炮', '鞭子', '鞭策'],
  '欺': ['欺负', '欺骗', '欺诈'],
  '挠': ['阻挠', '挠头', '抓挠'],
  '扳': ['扳手', '扳动', '扳平'],
  '腕': ['手腕', '腕力', '脚腕'],
  '剃': ['剃头', '剃刀', '剃须'],
  '腮': ['腮帮', '腮腺', '尖嘴猴腮'],
  '疤': ['疤痕', '伤疤', '疮疤'],
  '侄': ['侄子', '侄女', '叔侄'],
  '咙': ['喉咙', '咙喉', '嗓子眼'],
  '浆': ['豆浆', '泥浆', '糖浆'],
  '傅': ['师傅', '太傅', '傅雷'],
  '袱': ['包袱', '袱子', '搭袱'],
  '障': ['障碍', '保障', '故障'],
  '芝': ['芝麻', '灵芝', '芝士'],
  '圣': ['圣诞', '圣人', '神圣'],
  '馅': ['馅饼', '肉馅', '馅料'],
  '轰': ['轰动', '轰炸', '轰鸣'],
  '堪': ['难堪', '堪称', '疲惫不堪'],
  '诈': ['诈骗', '欺诈', '狡诈'],
  '捏': ['捏合', '捏造', '捏脸'],
  '怔': ['怔住', '发怔', '愣怔'],
  '矛': ['矛盾', '长矛', '矛头'],
  '盾': ['矛盾', '盾牌', '后盾'],
  '誉': ['荣誉', '名誉', '赞誉'],
  '吾': ['吾辈', '吾身', '支吾'],
  '嬴': ['嬴政', '嬴姓', '嬴秦'],
  '擦': ['擦拭', '擦洗', '摩擦'],
  '荐': ['推荐', '引荐', '自荐'],
  '艘': ['一艘', '客艘', '粮艘'],
  '航': ['航行', '航空', '导航'],
  '肆': ['放肆', '肆虐', '肆意'],
  '桅': ['桅杆', '船桅', '桅樯'],
  '撕': ['撕开', '撕毁', '撕扯'],
  '逗': ['逗留', '挑逗', '逗乐'],
  '唬': ['吓唬', '唬人', '诈唬'],
  '钩': ['钩子', '鱼钩', '挂钩'],
  '咧': ['咧嘴', '咧开', '大大咧咧'],
  '舱': ['船舱', '机舱', '客舱'],
  '鸥': ['海鸥', '沙鸥', '鸥鸟'],
  '瞄': ['瞄准', '瞄一眼', '偷瞄'],
  '尼': ['尼姑', '尼龙', '威尼斯'],
  '斯': ['斯文', '瓦斯', '莫斯科'],
  '艇': ['游艇', '潜艇', '舰艇'],
  '纵': ['纵横', '操纵', '放纵'],
  '艄': ['船艄', '艄公', '掌艄'],
  '翘': ['翘起', '翘首', '连翘'],
  '垫': ['垫子', '铺垫', '鞋垫'],
  '姆': ['保姆', '姆妈', '奶妈'],
  '祷': ['祷告', '祈祷', '祷祝'],
  '雇': ['雇佣', '雇主', '雇工'],
  '簇': ['簇拥', '一簇', '花簇'],
  '哗': ['喧哗', '哗啦', '哗然'],
  '码': ['号码', '密码', '码头'],
  '眺': ['眺望', '远眺', '凭眺'],
  '骏': ['骏马', '骏骑', '良骏'],
  '驰': ['奔驰', '驰骋', '飞驰'],
  '辽': ['辽阔', '辽宁', '辽远'],
  '绵': ['连绵', '绵羊', '绵延'],
  '凳': ['凳子', '板凳', '长凳'],
  '吆': ['吆喝', '吆唤', '吆五喝六'],
  '罐': ['罐子', '罐头', '瓦罐'],
  '恢': ['恢复', '恢弘', '法网恢恢'],
  '踢': ['踢球', '踢腿', '踢毽子'],
  '牲': ['牲畜', '牺牲', '牲口'],
  '畜': ['牲畜', '家畜', '畜牧'],
  '诣': ['造诣', '苦心孤诣', '诣见'],
  '禽': ['禽兽', '家禽', '飞禽'],
  '拇': ['拇指', '大拇指', '拇趾'],
  '搔': ['搔痒', '搔头', '搔扰'],
  '秽': ['污秽', '秽气', '秽物'],
  '轧': ['轧钢', '倾轧', '轧马路'],
  '拧': ['拧开', '拧紧', '拧毛巾'],
  '螺': ['螺丝', '海螺', '螺旋'],
  '纽': ['纽扣', '枢纽', '纽约'],
  '扣': ['扣子', '扣留', '折扣'],
  '仓': ['仓库', '仓促', '粮仓'],
  '庸': ['平庸', '庸俗', '庸人'],
  '憎': ['憎恨', '憎恶', '爱憎'],
  '毯': ['地毯', '毛毯', '毯子'],
  '裳': ['衣裳', '霓裳', '罗裳'],
  '蹄': ['马蹄', '猪蹄', '蹄子'],
  '腐': ['腐败', '腐烂', '豆腐'],
  '稍': ['稍微', '稍等', '稍后'],
  '缀': ['点缀', '连缀', '前缀'],
  '窥': ['窥视', '窥探', '偷窥'],
  '幽': ['幽静', '幽雅', '幽灵'],
  '案': ['案件', '方案', '案板'],
  '拙': ['笨拙', '拙劣', '拙见'],
  '薄': ['薄弱', '单薄', '薄雾'],
  '糊': ['模糊', '糊涂', '糊弄'],
  '蕾': ['花蕾', '蓓蕾', '芭蕾'],
  '恰': ['恰当', '恰好', '恰恰'],
  '襟': ['衣襟', '胸襟', '连襟'],
  '恍': ['恍惚', '恍然', '恍如'],
  '怨': ['怨恨', '抱怨', '埋怨'],
  '鹊': ['喜鹊', '鹊桥', '扁鹊'],
  '蝉': ['蝉鸣', '蝉蜕', '金蝉'],
  '崖': ['悬崖', '山崖', '崖壁'],
  '渡': ['渡河', '过渡', '渡口'],
  '索': ['搜索', '索取', '绳索'],
  '寇': ['日寇', '贼寇', '倭寇'],
  '副': ['副食', '副本', '副手'],
  '榴': ['石榴', '榴莲', '榴弹'],
  '抡': ['抡起', '抡刀', '抡拳'],
  '贯': ['贯彻', '贯穿', '贯通'],
  '悬': ['悬挂', '悬崖', '悬念'],
  '沸': ['沸腾', '沸水', '沸点'],
  '涧': ['山涧', '溪涧', '涧水'],
  '雹': ['冰雹', '雹子', '雹灾'],
  '屹': ['屹立', '屹然', '巍然屹立'],
  '悦': ['喜悦', '愉悦', '悦耳'],
  '迈': ['迈步', '年迈', '豪迈'],
  '屈': ['屈服', '委屈', '弯曲'],
  '政': ['政治', '政府', '政策'],
  '盏': ['一盏', '杯盏', '茶盏'],
  '栏': ['栏杆', '栏目', '栅栏'],
  '汇': ['汇合', '汇报', '外汇'],
  '宣': ['宣传', '宣布', '宣告'],
  '阅': ['阅读', '阅览', '检阅'],
  '制': ['制度', '制造', '限制'],
  '坦': ['平坦', '坦白', '坦然'],
  '距': ['距离', '差距', '相距'],
  '隆': ['隆重', '兴隆', '隆起'],
  '凛': ['凛冽', '凛然', '威风凛凛'],
  '疙': ['疙瘩', '疙疸', '疙疙瘩瘩'],
  '瘩': ['疙瘩', '瘩背', '鸡皮疙瘩'],
  '棍': ['棍子', '木棍', '冰棍'],
  '裁': ['裁剪', '裁判', '制裁'],
  '筹': ['筹备', '筹集', '一筹莫展'],
  '橡': ['橡皮', '橡树', '橡胶'],
  '雕': ['雕刻', '雕塑', '雕花'],
  '跺': ['跺脚', '跺足', '跺打'],
  '颓': ['颓废', '颓丧', '颓败'],
  '沮': ['沮丧', '沮遏', '沮其成行'],
  '丧': ['丧失', '沮丧', '丧事'],
  '趴': ['趴下', '趴着', '趴伏'],
  '屉': ['抽屉', '屉子', '笼屉'],
  '谜': ['谜语', '谜底', '灯谜'],
  '尚': ['高尚', '尚且', '时尚'],
  '氧': ['氧气', '氧化', '缺氧'],
  '倾': ['倾斜', '倾听', '倾向'],
  '揭': ['揭开', '揭示', '揭发'],
  '斑': ['斑点', '斑马', '斑驳'],
  '燥': ['干燥', '燥热', '枯燥'],
  '漠': ['沙漠', '冷漠', '漠然'],
  '磁': ['磁铁', '磁场', '磁性'],
  '盗': ['盗窃', '强盗', '海盗'],
  '培': ['培养', '培育', '培训'],
  '咆': ['咆哮', '咆吼', '咆哮如雷'],
  '哮': ['咆哮', '哮喘', '哮吼'],
  '哑': ['哑巴', '沙哑', '哑口无言'],
  '揪': ['揪住', '揪心', '揪出'],
  '瞪': ['瞪眼', '瞪视', '目瞪口呆'],
  '呻': ['呻吟', '呻唤', '无病呻吟'],
  '废': ['废物', '废除', '浪费'],
  '汹': ['汹涌', '汹汹', '气势汹汹'],
  '涌': ['涌现', '涌动', '汹涌'],
  '澎': ['澎湃', '澎湖', '心潮澎湃'],
  '湃': ['澎湃', '滂湃', '汹涌澎湃'],
  '熄': ['熄灭', '熄火', '熄灯'],
  '掀': ['掀起', '掀开', '掀翻'],
  '唉': ['唉声叹气', '唉呀', '唉唉'],
  '淋': ['淋雨', '淋湿', '淋浴'],
  '嘿': ['嘿嘿', '嘿哟', '嘿嘿笑'],
  '糟': ['糟糕', '糟蹋', '酒糟'],
  '嘛': ['干嘛', '好嘛', '是嘛'],
  '皱': ['皱纹', '皱眉', '褶皱'],
  '苔': ['苔藓', '青苔', '舌苔'],
  '藓': ['苔藓', '藓类', '泥炭藓'],
  '坪': ['草坪', '坪坝', '停机坪'],
  '蔗': ['甘蔗', '蔗糖', '蔗农'],
  '瀑': ['瀑布', '飞瀑', '瀑流'],
  '增': ['增加', '增长', '增强'],
  '缝': ['缝隙', '缝补', '裂缝'],
  '谚': ['谚语', '农谚', '古谚'],
  '袖': ['袖子', '袖口', '领袖'],
  '篷': ['帐篷', '篷布', '船篷'],
  '缩': ['缩小', '收缩', '退缩'],
  '疯': ['疯狂', '疯子', '发疯'],
  '柜': ['柜子', '衣柜', '柜台'],
  '喧': ['喧闹', '喧哗', '喧嚣'],
  '甩': ['甩开', '甩掉', '甩卖'],
  '嚷': ['叫嚷', '吵嚷', '嚷嚷'],
  '蒜': ['大蒜', '蒜苗', '蒜瓣'],
  '酱': ['酱油', '果酱', '炸酱'],
  '蹦': ['蹦跳', '蹦极', '蹦蹦跳跳'],
  '涯': ['天涯', '生涯', '无涯'],
  '莺': ['黄莺', '莺歌', '莺啼'],
  '莹': ['晶莹', '莹白', '清莹'],
  '裹': ['包裹', '裹住', '裹脚'],
  '篮': ['篮球', '篮子', '花篮'],
  '蔼': ['和蔼', '蔼然', '和蔼可亲'],
  '资': ['资源', '资本', '资料'],
  '矿': ['矿石', '煤矿', '矿产'],
  '慷': ['慷慨', '慷概', '慷慨激昂'],
  '慨': ['感慨', '慷慨', '慨叹'],
  '贡': ['贡献', '贡品', '进贡'],
  '滥': ['泛滥', '滥用', '滥竽充数'],
  '基': ['基础', '基本', '地基'],
  '睹': ['目睹', '先睹为快', '耳闻目睹'],
  '哉': ['善哉', '怪哉', '呜呼哀哉'],
  '巍': ['巍峨', '巍然', '巍然屹立'],
  '弦': ['琴弦', '弓弦', '弦乐'],
  '锦': ['锦绣', '锦缎', '锦旗'],
  '曝': ['曝光', '曝晒', '一曝十寒'],
  '矣': ['足矣', '悔之晚矣', '由来久矣'],
  '谱': ['乐谱', '菜谱', '谱写'],
  '茵': ['绿茵', '茵茵', '绿草如茵'],
  '盲': ['盲人', '盲目', '文盲'],
  '键': ['键盘', '按键', '关键'],
  '缕': ['一缕', '缕缕', '千丝万缕'],
  '郑': ['郑重', '郑州', '郑重其事'],
  '拜': ['拜访', '拜年', '崇拜'],
  '租': ['出租', '租房', '租金'],
  '毡': ['毡子', '毛毡', '如坐针毡'],
  '羞': ['害羞', '羞愧', '羞耻'],
  '撒': ['撒网', '撒谎', '撒种'],
  '缚': ['束缚', '绑缚', '作茧自缚'],
  '猬': ['刺猬', '猬集', '猬起'],
  '伶': ['伶俐', '伶仃', '名伶'],
  '俐': ['伶俐', '俏俐', '聪明伶俐'],
  '窜': ['逃窜', '窜改', '流窜'],
  '搁': ['搁置', '搁浅', '耽搁'],
  '综': ['综合', '综述', '错综'],
  '澄': ['澄清', '澄澈', '黄澄澄'],
  '萍': ['浮萍', '萍水相逢', '萍踪'],
  '漾': ['荡漾', '漾开', '碧波荡漾'],
  '削': ['削弱', '削减', '剥削'],
  '瞬': ['瞬间', '瞬息', '转瞬'],
  '凝': ['凝视', '凝聚', '凝固'],
  '骤': ['骤然', '步骤', '暴风骤雨'],
  '掷': ['投掷', '掷地有声', '抛掷'],
  '陡': ['陡峭', '陡然', '陡坡'],
  '摊': ['摊位', '摊开', '分摊'],
  '拌': ['搅拌', '拌饭', '凉拌'],
  '筝': ['风筝', '古筝', '筝弦'],
  '眨': ['眨眼', '眨巴', '一眨眼'],
  '宵': ['元宵', '宵夜', '通宵'],
  '燃': ['燃烧', '点燃', '燃料'],
  '戚': ['亲戚', '戚友', '休戚相关'],
  '贩': ['贩卖', '小贩', '商贩'],
  '彼': ['彼此', '彼岸', '知己知彼'],
  '贺': ['祝贺', '贺礼', '庆贺'],
  '轿': ['轿车', '花轿', '轿子'],
  '骆': ['骆驼', '骆马', '骆宾王'],
  '驼': ['骆驼', '驼峰', '驼背'],
  '腊': ['腊月', '腊肉', '腊肠'],
  '腻': ['细腻', '油腻', '腻烦'],
  '枣': ['红枣', '枣树', '枣子'],
  '搅': ['搅拌', '搅动', '搅乱'],
  '稠': ['稠密', '浓稠', '稠糊'],
  '肿': ['肿胀', '红肿', '水肿'],
  '熬': ['熬夜', '熬粥', '煎熬'],
  '褐': ['褐色', '褐煤', '短褐'],
  '缸': ['水缸', '鱼缸', '缸子'],
  '御': ['防御', '抵御', '御用'],
  '侯': ['诸侯', '侯爵', '王侯'],
  '皎': ['皎洁', '皎白', '皎月'],
  '泣': ['哭泣', '泣诉', '抽泣'],
  '栖': ['栖息', '栖身', '两栖'],
  '鸦': ['乌鸦', '鸦片', '鸦雀无声'],
  '凄': ['凄凉', '凄惨', '凄冷'],
  '寞': ['寂寞', '落寞', '孤寂'],
  '宴': ['宴会', '宴请', '晚宴'],
  '霉': ['发霉', '倒霉', '霉菌'],
  '籍': ['书籍', '国籍', '学籍'],
  '控': ['控制', '控告', '遥控'],
  '贷': ['贷款', '信贷', '借贷'],
  '剔': ['挑剔', '剔除', '剔透'],
  '毙': ['枪毙', '毙命', '束手待毙'],
  '抵': ['抵抗', '抵挡', '抵消'],
  '袭': ['袭击', '偷袭', '沿袭'],
  '覆': ['覆盖', '颠覆', '覆灭'],
  '藏': ['躲藏', '收藏', '宝藏'],
  '挪': ['挪动', '挪移', '挪用'],
  '徘': ['徘徊', '徘回', '徘徊不前'],
  '徊': ['徘徊', '徊徨', '徘徊不定'],
  '蒸': ['蒸发', '蒸汽', '蒸笼'],
  '裸': ['裸露', '裸体', '赤裸裸'],
  '媚': ['妩媚', '明媚', '谄媚'],
  '叨': ['唠叨', '叨扰', '念叨'],
  '绊': ['绊倒', '羁绊', '绊脚石'],
  '绞': ['绞刑', '绞尽脑汁', '绞痛'],
  '耽': ['耽误', '耽搁', '耽于享乐'],
  '揉': ['揉面', '揉搓', '揉眼睛'],
  '绽': ['绽放', '破绽', '绽开'],
  '沫': ['泡沫', '唾沫', '白沫'],
  '搓': ['搓手', '搓澡', '揉搓'],
  '惶': ['惶恐', '惶惑', '惊惶'],
  '吻': ['亲吻', '吻合', '口吻'],
  '偎': ['依偎', '偎依', '偎傍'],
  '络': ['网络', '脉络', '联络'],
  '凿': ['凿子', '开凿', '确凿'],
  '焚': ['焚烧', '焚毁', '心急如焚'],
  '稚': ['幼稚', '稚嫩', '稚气'],
  '避': ['避免', '躲避', '逃避'],
  '峻': ['严峻', '峻峭', '高峻'],
  '啪': ['啪嗒', '啪啪', '噼啪'],
  '僻': ['偏僻', '僻静', '孤僻'],
  '瞅': ['瞅见', '瞅空', '瞅一眼'],
  '靴': ['靴子', '皮靴', '雨靴'],
  '魔': ['魔鬼', '魔法', '魔术'],
  '哼': ['哼唱', '哼唧', '哼哼'],
  '绑': ['捆绑', '绑架', '绑带'],
  '啃': ['啃骨头', '啃书', '啃玉米'],
  '袍': ['长袍', '棉袍', '龙袍'],
  '押': ['抵押', '押送', '押金'],
  '执': ['执行', '执着', '执法'],
  '彻': ['彻底', '贯彻', '透彻'],
  '迁': ['迁移', '搬迁', '迁徙'],
  '泰': ['泰山', '泰国', '安泰'],
  '迫': ['强迫', '压迫', '迫切'],
  '批': ['批评', '批准', '批发'],
  '标': ['标准', '标志', '目标'],
  '牺': ['牺牲', '牺牛', '牺牲品'],
  '炊': ['炊烟', '炊事', '野炊'],
  '葬': ['埋葬', '葬礼', '安葬'],
  '援': ['援助', '支援', '救援'],
  '俱': ['俱全', '俱乐部', '面面俱到'],
  '弗': ['弗去', '自愧弗如', '弗许'],
  '辩': ['辩论', '辩解', '辩护'],
  '惯': ['习惯', '惯例', '惯性'],
  '圃': ['苗圃', '花圃', '菜圃'],
  '溅': ['飞溅', '溅起', '溅落'],
  '蕊': ['花蕊', '雄蕊', '雌蕊'],
  '搜': ['搜索', '搜查', '搜集'],
  '蚯': ['蚯蚓', '蚯螾', '蚯蚓洞'],
  '蚓': ['蚯蚓', '蚓窍', '蚯蚓粪'],
  '脆': ['清脆', '脆弱', '干脆'],
  '拦': ['阻拦', '拦截', '拦住'],
  '蟋': ['蟋蟀', '蟋蟋', '蟋蟀草'],
  '蟀': ['蟋蟀', '蟀谷', '斗蟋蟀'],
  '乙': ['乙方', '甲乙', '乙醇'],
  '匕': ['匕首', '图穷匕见', '匕鬯'],
  '刁': ['刁难', '刁蛮', '狡猾'],
  '乞': ['乞讨', '乞求', '乞丐'],
  '丸': ['丸子', '药丸', '弹丸'],
  '巳': ['巳时', '巳蛇', '辛巳'],
  '刃': ['刀刃', '利刃', '刃口'],
  '丐': ['乞丐', '丐帮', '乞讨'],
  '犬': ['犬子', '猎犬', '警犬'],
  '屯': ['屯兵', '屯粮', '村庄'],
  '戈': ['戈壁', '干戈', '反戈'],
  '壬': ['壬戌', '壬寅', '壬人'],
  '夭': ['夭折', '夭夭', '逃之夭夭'],
  '仁': ['仁义', '仁慈', '仁爱'],
  '仆': ['仆人', '公仆', '奴仆'],
  '丹': ['丹青', '牡丹', '丹田'],
  '亢': ['亢奋', '高亢', '不卑不亢'],
  '订': ['预订', '签订', '订正'],
  '冗': ['冗长', '冗杂', '冗余'],
  '讥': ['讥笑', '讥讽', '讥嘲'],
  '予': ['给予', '授予', '赋予'],
  '卉': ['花卉', '卉木', '奇花异卉'],
  '丙': ['丙等', '丙寅', '丙烯'],
  '夯': ['夯实', '夯土', '打夯'],
  '戊': ['戊戌', '戊寅', '戊申'],
  '凸': ['凸出', '凸起', '凹凸'],
  '卢': ['卢布', '卢沟桥', '姓卢'],
  '旦': ['元旦', '一旦', '旦夕'],
  '叭': ['喇叭', '叭叭', '叭嗒'],
  '叽': ['叽叽喳喳', '叽咕', '叽里咕噜'],
  '叼': ['叼着', '叼走', '叼烟'],
  '叩': ['叩门', '叩头', '叩拜'],
  '冉': ['冉冉', '姓冉', '冉升'],
  '皿': ['器皿', '皿器', '金皿'],
  '凹': ['凹陷', '凹进', '凹凸'],
  '矢': ['矢志', '有的放矢', '箭矢'],
  '乍': ['乍看', '乍暖还寒', '乍一'],
  '卯': ['卯时', '卯兔', '点卯'],
  '冯': ['姓冯', '冯巩', '冯唐'],
  '玄': ['玄妙', '玄机', '玄乎'],
  '汁': ['果汁', '汁液', '汁水'],
  '讯': ['通讯', '审讯', '喜讯'],
  '弘': ['弘扬', '恢弘', '弘愿'],
  '奴': ['奴隶', '奴仆', '农奴'],
  '孕': ['怀孕', '孕育', '孕妇'],
  '邦': ['邦交', '联邦', '邻邦'],
  '迂': ['迂回', '迂腐', '迂缓'],
  '巩': ['巩固', '巩俐', '巩膜'],
  '芋': ['芋头', '洋芋', '芋艿'],
  '朽': ['腐朽', '不朽', '朽木'],
  '吏': ['官吏', '吏治', '酷吏'],
  '戌': ['戌时', '戊戌', '戌狗'],
  '夷': ['夷为平地', '鄙夷', '蛮夷'],
  '轨': ['轨道', '铁轨', '正轨'],
  '邪': ['邪恶', '邪气', '辟邪'],
  '尧': ['尧舜', '尧天', '唐尧'],
  '贞': ['忠贞', '贞洁', '贞观'],
  '吁': ['呼吁', '吁请', '长吁短叹'],
  '吕': ['吕布', '吕宋', '姓吕'],
  '屿': ['岛屿', '鼓浪屿', '屿礁'],
  '廷': ['朝廷', '宫廷', '廷议'],
  '乔': ['乔木', '乔迁', '姓乔'],
  '迄': ['迄今', '迄未', '起迄'],
  '乒': ['乒乓球', '乒乓', '乒坛'],
  '乓': ['乒乓球', '乒乓', '乓乓'],
  '臼': ['臼齿', '石臼', '脱臼'],
  '伐': ['砍伐', '讨伐', '步伐'],
  '仲': ['仲裁', '仲夏', '仲秋'],
  '伪': ['虚伪', '伪造', '伪善'],
  '伊': ['伊人', '伊始', '伊斯兰教'],
  '兆': ['征兆', '兆头', '万亿'],
  '企': ['企业', '企图', '企盼'],
  '肌': ['肌肉', '肌肤', '心肌'],
  '肋': ['肋骨', '肋条', '鸡肋'],
  '旬': ['上旬', '中旬', '下旬'],
  '旨': ['主旨', '旨意', '宗旨'],
  '旭': ['旭日', '旭光', '旭日东升'],
  '匈': ['匈奴', '匈牙利', '匈人'],
  '妄': ['妄想', '狂妄', '妄自菲薄'],
  '汝': ['汝辈', '汝等', '汝州'],
  '讳': ['忌讳', '讳言', '避讳'],
  '讹': ['讹诈', '讹传', '以讹传讹'],
  '讼': ['诉讼', '讼棍', '争讼'],
  '讽': ['讽刺', '讥讽', '讽喻'],
  '诀': ['诀窍', '秘诀', '诀别'],
  '弛': ['松弛', '弛缓', '一张一弛'],
  '奸': ['奸诈', '汉奸', '奸商'],
  '妃': ['妃子', '贵妃', '王妃'],
  '驮': ['驮运', '驮着', '驮马'],
  '驯': ['驯服', '驯良', '驯养'],
  '纫': ['缝纫', '纫针', '感纫'],
  '巡': ['巡逻', '巡视', '巡回'],
  '玖': ['玖月', '玖拾', '佩玖'],
  '玛': ['玛瑙', '玛雅', '玛丽'],
  '抚': ['抚摸', '抚养', '抚慰'],
  '抠': ['抠门', '抠图', '抠鼻子'],
  '扰': ['打扰', '扰乱', '骚扰'],
  '扼': ['扼杀', '扼要', '扼腕'],
  '汞': ['汞灯', '汞柱', '水银'],
  '孝': ['孝顺', '孝心', '孝敬'],
  '坎': ['坎坷', '坎肩', '田坎'],
  '坟': ['坟墓', '坟地', '上坟'],
  '坊': ['作坊', '街坊', '牌坊'],
  '抒': ['抒发', '抒情', '抒写'],
  '芜': ['荒芜', '芜杂', '繁芜'],
  '苇': ['芦苇', '苇塘', '苇席'],
  '芹': ['芹菜', '芹献', '水芹'],
  '芥': ['芥末', '芥菜', '草芥'],
  '芯': ['芯片', '灯芯', '笔芯'],
  '芭': ['芭蕉', '芭蕾', '芭比'],
  '杠': ['杠杆', '杠铃', '单杠'],
  '甫': ['杜甫', '甫定', '神甫'],
  '酉': ['酉时', '酉鸡', '酉阳'],
  '辰': ['时辰', '辰龙', '诞辰'],
  '励': ['鼓励', '激励', '励志'],
  '尬': ['尴尬', '尬聊', '尬舞'],
  '歼': ['歼灭', '歼敌', '围歼'],
  '卤': ['卤水', '卤味', '盐卤'],
  '肖': ['肖像', '生肖', '姓肖'],
  '吱': ['吱吱', '吱声', '嘎吱'],
  '吠': ['犬吠', '吠叫', '狂吠'],
  '呕': ['呕吐', '呕心沥血', '干呕'],
  '呛': ['呛水', '呛人', '呛咳'],
  '呜': ['呜咽', '呜呜', '呜呼'],
  '邑': ['城邑', '邑人', '都邑'],
  '囤': ['囤积', '囤货', '粮囤'],
  '吮': ['吮吸', '吮乳', '吸吮'],
  '岖': ['崎岖', '岖崟', '岩岖'],
  '牡': ['牡丹', '牡蛎', '牡牛'],
  '秃': ['秃头', '秃鹫', '光秃秃'],
  '佐': ['辅佐', '佐料', '佐证'],
  '佑': ['保佑', '庇佑', '护佑'],
  '佃': ['佃户', '佃农', '租佃'],
  '佣': ['佣人', '佣金', '雇佣'],
  '伺': ['伺候', '伺机', '窥伺'],
  '囱': ['烟囱', '囱门', '囱灶'],
  '役': ['兵役', '劳役', '战役'],
  '妥': ['妥当', '妥善', '妥协'],
  '岔': ['岔路', '岔开', '岔道'],
  '肛': ['肛门', '肛裂', '肛瘘'],
  '龟': ['乌龟', '龟甲', '海龟'],
  '甸': ['沉甸甸', '甸子', '伊甸园'],
  '狈': ['狼狈', '狼狈为奸', '狼狈不堪'],
  '删': ['删除', '删改', '删减'],
  '彤': ['红彤彤', '彤云', '彤管'],
  '灸': ['针灸', '艾灸', '灸疗'],
  '刨': ['刨子', '刨冰', '刨根问底'],
  '庇': ['庇护', '庇佑', '包庇'],
  '吝': ['吝啬', '吝惜', '不吝赐教'],
  '冶': ['冶炼', '冶金', '陶冶'],
  '闰': ['闰年', '闰月', '闰日'],
  '判': ['判断', '判决', '批判'],
  '兑': ['兑换', '兑现', '兑奖'],
  '灼': ['灼热', '灼伤', '灼灼'],
  '沐': ['沐浴', '沐恩', '栉风沐雨'],
  '沛': ['充沛', '沛然', '丰沛'],
  '汰': ['淘汰', '汰渍', '优胜劣汰'],
  '沥': ['沥青', '沥血', '淅沥'],
  '沦': ['沦陷', '沦落', '沉沦'],
  '沧': ['沧桑', '沧海', '沧州'],
  '沪': ['沪上', '沪宁', '淞沪'],
  '沈': ['沈阳', '沈括', '姓沈'],
  '沁': ['沁人心脾', '沁园春', '沁润'],
  '忱': ['热忱', '忱谢', '赤忱'],
  '灾': ['灾难', '灾害', '火灾'],
  '祀': ['祭祀', '祀天', '奉祀'],
  '译': ['翻译', '译文', '译制'],
  '陈': ['陈旧', '陈述', '姓陈'],
  '妓': ['妓女', '妓院', '歌妓'],
  '姊': ['姊妹', '姊弟', '姊姊'],
  '纬': ['纬度', '经纬', '纬线'],
  '驱': ['驱赶', '驱动', '驱逐'],
  '驳': ['反驳', '驳斥', '斑驳'],
  '玫': ['玫瑰', '玫红', '玫丽'],
  '卦': ['八卦', '算卦', '变卦'],
  '坷': ['坎坷', '坷垃', '土坷垃'],
  '坯': ['土坯', '坯子', '毛坯'],
  '拓': ['开拓', '拓展', '拓扑'],
  '拣': ['挑拣', '拣选', '拣拾'],
  '坤': ['乾坤', '坤包', '坤角'],
  '拎': ['拎包', '拎着', '拎水'],
  '拄': ['拄拐', '拄杖', '拄着'],
  '拗': ['执拗', '拗口', '拗断'],
  '茉': ['茉莉', '茉莉花', '茉香'],
  '昔': ['昔日', '往昔', '今昔'],
  '苛': ['苛刻', '苛求', '苛捐杂税'],
  '苑': ['花苑', '苑囿', '文苑'],
  '苞': ['花苞', '苞米', '含苞'],
  '茁': ['茁壮', '茁实', '茁长'],
  '枉': ['冤枉', '枉费', '枉然'],
  '矾': ['明矾', '矾土', '红矾'],
  '殴': ['殴打', '斗殴', '殴伤'],
  '垄': ['垄断', '田垄', '瓦垄'],
  '顷': ['顷刻', '公顷', '少顷'],
  '歧': ['歧视', '分歧', '歧途'],
  '卓': ['卓越', '卓著', '卓见'],
  '贤': ['贤明', '贤惠', '贤才'],
  '昌': ['昌盛', '昌明', '繁荣昌盛'],
  '畅': ['畅通', '畅快', '畅谈'],
  '咒': ['诅咒', '咒语', '咒骂'],
  '咋': ['咋办', '咋了', '咋整'],
  '咏': ['歌咏', '咏叹', '咏春'],
  '咄': ['咄咄逼人', '咄嗟', '咄咄'],
  '帖': ['请帖', '字帖', '服帖'],
  '帜': ['旗帜', '标帜', '易帜'],
  '帕': ['手帕', '帕子', '帕米尔'],
  '账': ['账目', '账本', '算账'],
  '贬': ['贬低', '贬值', '褒贬'],
  '贮': ['贮藏', '贮存', '贮备'],
  '迭': ['迭代', '迭起', '更迭'],
  '氛': ['气氛', '氛围', '氛霭'],
  '秆': ['秸秆', '麦秆', '麻秆'],
  '秉': ['秉承', '秉公', '秉烛'],
  '侠': ['侠客', '侠义', '武侠'],
  '侥': ['侥幸', '侥求', '侥利'],
  '侦': ['侦查', '侦探', '侦缉'],
  '侣': ['伴侣', '情侣', '僧侣'],
  '侨': ['华侨', '侨胞', '侨民'],
  '侈': ['奢侈', '侈谈', '侈靡'],
  '卑': ['卑微', '卑鄙', '自卑'],
  '肴': ['菜肴', '佳肴', '酒肴'],
  '觅': ['寻觅', '觅食', '觅友'],
  '忿': ['忿恨', '忿怒', '不忿'],
  '肤': ['皮肤', '肤浅', '肌肤'],
  '肮': ['肮脏', '肮胀', '肮里肮脏'],
  '肪': ['脂肪', '肪厚', '膏肪'],
  '胁': ['威胁', '胁迫', '胁从'],
  '狞': ['狰狞', '狞笑', '狞恶'],
  '饲': ['饲养', '饲料', '饲育'],
  '疟': ['疟疾', '疟原虫', '疟蚊'],
  '疚': ['内疚', '负疚', '愧疚'],
  '卒': ['卒业', '小卒', '兵卒'],
  '庚': ['庚帖', '庚辰', '贵庚'],
  '氓': ['流氓', '氓隶', '愚氓'],
  '闸': ['闸门', '闸口', '水闸'],
  '券': ['证券', '债券', '入场券'],
  '炬': ['火炬', '炬火', '目光如炬'],
  '泄': ['泄露', '发泄', '排泄'],
  '沽': ['沽名', '沽酒', '待价而沽'],
  '泞': ['泥泞', '泞淖', '泞滑'],
  '泌': ['分泌', '泌尿', '泌乳'],
  '沼': ['沼泽', '沼气', '池沼'],
  '泽': ['光泽', '恩泽', '沼泽'],
  '怖': ['恐怖', '怖惧', '惊怖'],
  '宠': ['宠爱', '宠物', '宠儿'],
  '宛': ['宛如', '宛然', '婉转'],
  '祈': ['祈祷', '祈求', '祈愿'],
  '诞': ['诞生', '诞辰', '荒诞'],
  '诡': ['诡计', '诡异', '诡辩'],
  '询': ['询问', '咨询', '查询'],
  '隶': ['奴隶', '隶属', '隶书'],
  '帚': ['扫帚', '帚子', '敝帚自珍'],
  '届': ['届时', '应届', '换届'],
  '弧': ['弧线', '弧度', '弧形'],
  '弥': ['弥漫', '弥补', '弥留'],
  '孟': ['孟子', '孟春', '姓孟'],
  '陋': ['简陋', '丑陋', '陋习'],
  '陕': ['陕西', '陕北', '陕南'],
  '函': ['函数', '信函', '函件'],
  '妮': ['妮子', '妮妮', '小妮'],
  '迢': ['迢迢', '迢远', '千里迢迢'],
  '叁': ['叁佰', '叁仟', '叁元'],
  '绅': ['绅士', '绅商', '缙绅'],
  '驹': ['马驹', '驹子', '白驹过隙'],
  '驻': ['驻扎', '驻守', '驻足'],
  '绎': ['演绎', '络绎', '绎思'],
  '契': ['契约', '契合', '契机'],
  '贰': ['贰佰', '贰心', '贰元'],
  '玷': ['玷污', '玷辱', '瑕疵'],
  '珊': ['珊瑚', '珊珊', '阑珊'],
  '拷': ['拷贝', '拷问', '拷打'],
  '垮': ['垮台', '垮掉', '打垮'],
  '挎': ['挎包', '挎着', '挎篮'],
  '挟': ['要挟', '挟持', '挟制'],
  '垢': ['污垢', '垢污', '藏污纳垢'],
  '垛': ['垛子', '草垛', '城墙垛'],
  '拯': ['拯救', '拯济', '拯溺'],
  '茬': ['找茬', '茬口', '麦茬'],
  '茧': ['蚕茧', '茧子', '老茧'],
  '荤': ['荤菜', '荤腥', '开荤'],
  '荧': ['荧光', '荧幕', '荧屏'],
  '荫': ['树荫', '荫蔽', '荫凉'],
  '荔': ['荔枝', '荔波', '荔园'],
  '栈': ['客栈', '栈道', '货栈'],
  '柑': ['柑橘', '柑子', '蜜柑'],
  '栅': ['栅栏', '栅门', '铁栅'],
  '柿': ['柿子', '柿饼', '西红柿'],
  '柬': ['请柬', '柬帖', '书柬'],
  '咸': ['咸味', '咸菜', '咸水'],
  '砂': ['砂石', '砂锅', '砂纸'],
  '泵': ['水泵', '泵站', '气泵'],
  '轴': ['轴承', '轴线', '压轴'],
  '皆': ['皆是', '皆大欢喜', '比比皆是'],
  '韭': ['韭菜', '韭黄', '韭花'],
  '虐': ['虐待', '肆虐', '虐杀'],
  '昧': ['冒昧', '愚昧', '昧心'],
  '盹': ['打盹', '盹睡', '盹儿'],
  '昭': ['昭然', '昭示', '昭雪'],
  '畏': ['畏惧', '敬畏', '畏缩'],
  '勋': ['功勋', '勋章', '勋业'],
  '哆': ['哆嗦', '哆啦A梦', '哆口'],
  '咪': ['咪咪', '咪表', '咪猫'],
  '哟': ['哎哟', '哟呵', '嗨哟'],
  '贻': ['贻误', '贻害', '贻笑大方'],
  '钙': ['钙片', '钙质', '补钙'],
  '钞': ['钞票', '钞券', '现钞'],
  '钠': ['钠盐', '钠灯', '氯化钠'],
  '钦': ['钦佩', '钦差', '钦定'],
  '钧': ['千钧', '钧瓷', '钧旨'],
  '钮': ['按钮', '钮扣', '电钮'],
  '卸': ['卸货', '卸载', '推卸'],
  '氢': ['氢气', '氢弹', '氢氧'],
  '复': ['复习', '复杂', '恢复'],
  '俏': ['俏皮', '俏丽', '俊俏'],
  '俄': ['俄罗斯', '俄顷', '俄而'],
  '侮': ['侮辱', '侮蔑', '欺侮'],
  '俭': ['节俭', '俭朴', '勤俭'],
  '禹': ['大禹', '禹州', '禹贡'],
  '俊': ['英俊', '俊俏', '俊杰'],
  '衍': ['繁衍', '衍生', '敷衍'],
  '叙': ['叙述', '叙旧', '记叙'],
  '胚': ['胚胎', '胚芽', '胚层'],
  '狰': ['狰狞', '狰猛', '狰狞'],
  '狡': ['狡猾', '狡诈', '狡辩'],
  '贸': ['贸易', '贸然', '商贸'],
  '饵': ['鱼饵', '诱饵', '饵食'],
  '蚀': ['腐蚀', '侵蚀', '蚀本'],
  '峦': ['山峦', '峰峦', '峦嶂'],
  '疮': ['疮疤', '疮口', '冻疮'],
  '疫': ['瘟疫', '疫苗', '防疫'],
  '咨': ['咨询', '咨文', '咨商'],
  '闺': ['闺女', '闺房', '闺阁'],
  '闽': ['福建', '闽南', '闽菜'],
  '阀': ['阀门', '军阀', '门阀'],
  '姜': ['生姜', '姜汤', '姜黄'],
  '叛': ['背叛', '叛乱', '叛徒'],
  '娄': ['娄子', '姓娄', '娄底'],
  '逆': ['叛逆', '逆流', '逆境'],
  '兹': ['兹有', '兹事', '念兹在兹'],
  '炫': ['炫耀', '炫目', '炫技'],
  '柒': ['柒佰', '柒元', '柒拾'],
  '洽': ['洽谈', '融洽', '接洽'],
  '浏': ['浏览', '浏阳', '清浏'],
  '济': ['经济', '救济', '同舟共济'],
  '恒': ['永恒', '恒心', '恒定'],
  '恬': ['恬静', '恬然', '恬不知耻'],
  '恤': ['体恤', '抚恤', '怜恤'],
  '恼': ['烦恼', '恼怒', '苦恼'],
  '宦': ['宦官', '宦海', '官宦'],
  '宪': ['宪法', '宪兵', '立宪'],
  '窃': ['盗窃', '窃取', '窃听'],
  '诫': ['告诫', '诫勉', '训诫'],
  '诬': ['诬陷', '诬蔑', '诬告'],
  '袄': ['棉袄', '皮袄', '夹袄'],
  '祠': ['祠堂', '祠庙', '宗祠'],
  '垦': ['开垦', '垦荒', '农垦'],
  '陨': ['陨石', '陨落', '陨灭'],
  '姥': ['姥姥', '姥爷', '天姥'],
  '姻': ['婚姻', '姻缘', '姻亲'],
  '姚': ['姚明', '姓姚', '姚黄'],
  '娜': ['婀娜', '娜娜', '袅娜'],
  '怠': ['懈怠', '怠慢', '怠工'],
  '癸': ['癸丑', '癸未', '天癸'],
  '蚤': ['跳蚤', '蚤子', '水蚤'],
  '垒': ['堡垒', '垒球', '壁垒'],
  '绚': ['绚丽', '绚烂', '绚美'],
  '骇': ['惊骇', '骇人听闻', '骇异'],
  '耗': ['消耗', '耗费', '噩耗'],
  '耙': ['耙子', '耙地', '钉耙'],
  '匿': ['匿名', '藏匿', '隐匿'],
  '埂': ['田埂', '埂子', '土埂'],
  '捂': ['捂住', '捂嘴', '捂热'],
  '捎': ['捎带', '捎信', '捎话'],
  '捍': ['捍卫', '捍御', '捍护'],
  '捆': ['捆绑', '捆扎', '一捆'],
  '捐': ['捐款', '捐赠', '捐献'],
  '袁': ['袁世凯', '姓袁', '袁隆平'],
  '捌': ['捌佰', '捌元', '捌拾'],
  '逝': ['逝世', '逝去', '流逝'],
  '挫': ['挫折', '挫败', '挫伤'],
  '挚': ['真挚', '挚爱', '挚友'],
  '捣': ['捣乱', '捣鼓', '捣碎'],
  '捅': ['捅破', '捅娄子', '捅马蜂窝'],
  '埃': ['尘埃', '埃及', '埃尘'],
  '耿': ['耿直', '耿耿于怀', '耿介'],
  '聂': ['聂耳', '姓聂', '聂荣臻'],
  '莽': ['鲁莽', '莽原', '草莽'],
  '莱': ['莱茵河', '莱芜', '蓬莱'],
  '莉': ['茉莉', '莉莉', '莉香'],
  '梆': ['梆子', '梆梆', '硬梆梆'],
  '桔': ['桔子', '桔梗', '桔槔'],
  '档': ['档案', '档次', '搭档'],
  '栗': ['栗子', '板栗', '战栗'],
  '酌': ['斟酌', '酌量', '小酌'],
  '辱': ['侮辱', '耻辱', '荣辱'],
  '砰': ['砰砰', '砰然', '砰的一声'],
  '砾': ['砾石', '瓦砾', '砂砾'],
  '础': ['基础', '础石', '柱础'],
  '殉': ['殉职', '殉国', '殉情'],
  '逞': ['逞能', '逞强', '得逞'],
  '唠': ['唠叨', '唠嗑', '唠扯'],
  '哺': ['哺乳', '哺育', '哺养'],
  '蚌': ['蚌壳', '河蚌', '鹬蚌相争'],
  '蚣': ['蜈蚣', '蚣蝮', '蚣虫'],
  '蚪': ['蝌蚪', '蚪虫', '蚪尾'],
  '唁': ['吊唁', '唁电', '慰唁'],
  '唧': ['唧唧', '唧筒', '哼唧'],
  '唆': ['教唆', '唆使', '挑唆'],
  '峭': ['陡峭', '峭壁', '峭立'],
  '峨': ['巍峨', '峨眉', '峨冠'],
  '贿': ['贿赂', '行贿', '受贿'],
  '赂': ['贿赂', '赂遗', '财赂'],
  '赃': ['赃物', '赃款', '分赃'],
  '钳': ['钳子', '钳制', '钳工'],
  '钾': ['钾肥', '钾盐', '氯化钾'],
  '氨': ['氨气', '氨水', '氨基酸'],
  '债': ['债务', '欠债', '还债'],
  '倚': ['倚靠', '倚仗', '倚老卖老'],
  '倡': ['提倡', '倡导', '倡议'],
  '赁': ['租赁', '赁屋', '出赁'],
  '躬': ['鞠躬', '躬身', '躬行'],
  '倔': ['倔强', '倔脾气', '倔头倔脑'],
  '舰': ['军舰', '舰队', '舰艇'],
  '舀': ['舀水', '舀汤', '舀米'],
  '豺': ['豺狼', '豺狗', '豺豹'],
  '颁': ['颁布', '颁发', '颁奖'],
  '颂': ['歌颂', '颂扬', '赞颂'],
  '胰': ['胰腺', '胰岛素', '胰液'],
  '脐': ['肚脐', '脐带', '脐橙'],
  '脓': ['脓包', '脓液', '化脓'],
  '卿': ['卿卿', '公卿', '爱卿'],
  '鸵': ['鸵鸟', '鸵绒', '鸵蛋'],
  '馁': ['气馁', '馁怯', '冻馁'],
  '郭': ['城郭', '姓郭', '郭沫若'],
  '症': ['症状', '病症', '症结'],
  '疹': ['皮疹', '疹子', '麻疹'],
  '紊': ['紊乱', '紊杂', '有条不紊'],
  '羔': ['羔羊', '羔皮', '羊羔'],
  '烘': ['烘烤', '烘托', '烘干'],
  '烙': ['烙印', '烙饼', '烙铁'],
  '涝': ['洪涝', '涝灾', '防涝'],
  '浦': ['浦口', '黄浦', '浦滩'],
  '涉': ['涉及', '跋涉', '干涉'],
  '涡': ['漩涡', '涡轮', '涡流'],
  '浩': ['浩瀚', '浩荡', '浩渺'],
  '涣': ['涣散', '涣然', '涣散'],
  '涤': ['洗涤', '涤荡', '清涤'],
  '涩': ['苦涩', '羞涩', '干涩'],
  '悖': ['悖论', '悖逆', '并行不悖'],
  '悟': ['领悟', '觉悟', '醒悟'],
  '悍': ['强悍', '凶悍', '悍然'],
  '悯': ['怜悯', '悯惜', '悲天悯人'],
  '窍': ['窍门', '诀窍', '七窍'],
  '诽': ['诽谤', '诽议', '腹诽'],
  '冥': ['冥想', '冥思', '幽冥'],
  '冤': ['冤枉', '冤屈', '冤案'],
  '谅': ['原谅', '谅解', '体谅'],
  '谆': ['谆谆', '谆嘱', '谆谆教诲'],
  '恳': ['诚恳', '恳切', '恳求'],
  '祟': ['作祟', '祟恶', '鬼祟'],
  '娟': ['娟秀', '婵娟', '娟丽'],
  '恕': ['宽恕', '饶恕', '恕不奉陪'],
  '娥': ['嫦娥', '娥眉', '宫娥'],
  '绢': ['手绢', '绢花', '绢本'],
  '琉': ['琉璃', '琉球', '琉瓦'],
  '琅': ['琳琅', '琅琊', '琅琅'],
  '捺': ['捺印', '捺住', '按捺'],
  '焉': ['心不在焉', '焉能', '焉知'],
  '赦': ['赦免', '赦罪', '大赦'],
  '埠': ['商埠', '埠头', '外埠'],
  '授': ['教授', '授予', '传授'],
  '捻': ['捻线', '捻子', '灯捻'],
  '掂': ['掂量', '掂掇', '掂算'],
  '掺': ['掺和', '掺杂', '掺假'],
  '聆': ['聆听', '聆教', '聆讯'],
  '勘': ['勘察', '勘探', '勘误'],
  '菱': ['菱形', '菱角', '菱花'],
  '勒': ['勒索', '勒紧', '勾勒'],
  '菲': ['芳菲', '菲薄', '菲律宾'],
  '萌': ['萌芽', '萌发', '萌生'],
  '菩': ['菩萨', '菩提', '菩提树'],
  '菠': ['菠菜', '菠萝', '菠萝蜜'],
  '营': ['营养', '经营', '军营'],
  '乾': ['乾坤', '乾隆', '乾陵'],
  '萧': ['萧条', '萧瑟', '姓萧'],
  '萨': ['菩萨', '拉萨', '萨满'],
  '彬': ['彬彬有礼', '彬雅', '文质彬彬'],
  '婪': ['贪婪', '婪索', '婪酣'],
  '梗': ['梗概', '梗塞', '作梗'],
  '梧': ['梧桐', '魁梧', '梧州'],
  '梭': ['穿梭', '梭子', '梭镖'],
  '酝': ['酝酿', '酝藉', '酝造'],
  '酗': ['酗酒', '酗饮', '酗骂'],
  '厢': ['车厢', '厢房', '包厢'],
  '硅': ['硅谷', '硅胶', '硅酸盐'],
  '奢': ['奢侈', '奢华', '奢望'],
  '盔': ['盔甲', '头盔', '钢盔'],
  '聋': ['聋哑', '耳聋', '装聋作哑'],
  '匾': ['匾额', '牌匾', '横匾'],
  '辅': ['辅助', '辅导', '辅佐'],
  '彪': ['彪悍', '彪炳', '彪形大汉'],
  '眶': ['眼眶', '热泪盈眶', '眶睫'],
  '眯': ['眯眼', '眯缝', '笑眯眯'],
  '曼': ['曼妙', '曼延', '曼谷'],
  '晦': ['晦暗', '晦气', '晦涩'],
  '啄': ['啄木鸟', '啄食', '啄米'],
  '趾': ['脚趾', '趾高气扬', '趾骨'],
  '蛀': ['蛀牙', '蛀虫', '蛀蚀'],
  '鄂': ['鄂伦春', '鄂州', '鄂西'],
  '患': ['患者', '患难', '隐患'],
  '啰': ['啰嗦', '啰唆', '喽啰'],
  '啤': ['啤酒', '啤酒店', '生啤'],
  '啥': ['啥子', '啥事', '为啥'],
  '崎': ['崎岖', '崎崟', '崎岭'],
  '逻': ['巡逻', '逻辑', '逻骑'],
  '崔': ['崔巍', '姓崔', '崔嵬'],
  '帷': ['帷幕', '帷幔', '运筹帷幄'],
  '崛': ['崛起', '崛立', '崛兴'],
  '铐': ['手铐', '镣铐', '铐住'],
  '铭': ['铭记', '铭文', '座右铭'],
  '矫': ['矫正', '矫健', '矫情'],
  '秸': ['秸秆', '麦秸', '麻秸'],
  '犁': ['犁地', '犁铧', '犁田'],
  '笙': ['笙箫', '笙歌', '芦笙'],
  '偿': ['赔偿', '偿还', '补偿'],
  '售': ['销售', '出售', '零售'],
  '兜': ['兜风', '兜售', '衣兜'],
  '衅': ['挑衅', '衅端', '寻衅'],
  '徙': ['迁徙', '徙居', '流徙'],
  '舶': ['船舶', '舶来品', '海舶'],
  '舵': ['舵手', '掌舵', '舵轮'],
  '敛': ['收敛', '敛财', '敛迹'],
  '脯': ['胸脯', '果脯', '肉脯'],
  '豚': ['海豚', '河豚', '豚鼠'],
  '凰': ['凤凰', '凰鸟', '凤求凰'],
  '猖': ['猖獗', '猖狂', '猖披'],
  '烹': ['烹饪', '烹调', '烹茶'],
  '庶': ['富庶', '庶民', '庶几'],
  '庵': ['尼姑庵', '庵堂', '茅庵'],
  '痊': ['痊愈', '痊可', '痊济'],
  '阎': ['阎王', '阎罗', '姓阎'],
  '阐': ['阐述', '阐明', '阐释'],
  '羚': ['羚羊', '羚牛', '羚角'],
  '焊': ['焊接', '焊工', '电焊'],
  '焕': ['焕发', '焕然一新', '焕炳'],
  '鸿': ['鸿雁', '鸿沟', '鸿运'],
  '渠': ['渠道', '沟渠', '水渠'],
  '淑': ['淑女', '淑静', '贤淑'],
  '淮': ['淮河', '淮南', '淮北'],
  '淆': ['混淆', '淆乱', '淆惑'],
  '渊': ['渊博', '深渊', '渊源'],
  '淫': ['淫雨', '淫秽', '淫威'],
  '淳': ['淳朴', '淳厚', '淳于'],
  '淤': ['淤泥', '淤积', '淤血'],
  '淀': ['淀粉', '沉淀', '白洋淀'],
  '涮': ['涮羊肉', '涮洗', '涮锅'],
  '涵': ['涵养', '内涵', '包涵'],
  '惭': ['惭愧', '惭怍', '羞惭'],
  '悼': ['悼念', '悼词', '哀悼'],
  '惕': ['警惕', '惕厉', '惕惧'],
  '惟': ['惟独', '惟恐', '惟妙惟肖'],
  '惦': ['惦记', '惦念', '惦挂'],
  '悴': ['憔悴', '悴薄', '愁悴'],
  '惋': ['惋惜', '惋叹', '怅惋'],
  '寅': ['寅时', '寅虎', '寅吃卯粮'],
  '窒': ['窒息', '窒碍', '窒闷'],
  '窑': ['窑洞', '窑厂', '砖窑'],
  '谍': ['间谍', '谍报', '谍战'],
  '祸': ['灾祸', '祸患', '闯祸'],
  '逮': ['逮捕', '逮住', '力有未逮'],
  '尉': ['上尉', '中尉', '少尉'],
  '屠': ['屠杀', '屠夫', '屠宰'],
  '隋': ['隋朝', '隋代', '姓隋'],
  '堕': ['堕落', '堕胎', '堕入'],
  '隅': ['角落', '一隅', '城隅'],
  '婉': ['婉转', '委婉', '婉言'],
  '琳': ['琳琅', '琳珉', '琳琅满目'],
  '琢': ['琢磨', '雕琢', '琢玉'],
  '琼': ['琼浆', '琼楼', '琼瑶'],
  '揍': ['揍人', '挨揍', '揍一顿'],
  '款': ['款式', '款项', '款待'],
  '堰': ['堤坝', '堰塘', '都江堰'],
  '揩': ['揩油', '揩拭', '揩汗'],
  '趋': ['趋势', '趋向', '趋炎附势'],
  '揽': ['招揽', '揽活', '包揽'],
  '揣': ['揣测', '揣摩', '揣度'],
  '搀': ['搀扶', '搀和', '搀假'],
  '壹': ['壹佰', '壹元', '壹拾'],
  '募': ['募捐', '募集', '招募'],
  '董': ['董事', '古董', '姓董'],
  '葱': ['大葱', '葱花', '葱郁'],
  '蒋': ['蒋介石', '姓蒋', '蒋氏'],
  '蒂': ['瓜蒂', '根深蒂固', '芥蒂'],
  '韩': ['韩国', '韩信', '姓韩'],
  '辜': ['辜负', '无辜', '死有余辜'],
  '棱': ['棱角', '棱镜', '模棱两可'],
  '椰': ['椰子', '椰树', '椰汁'],
  '椒': ['辣椒', '花椒', '胡椒'],
  '椎': ['颈椎', '腰椎', '脊椎'],
  '棺': ['棺材', '棺木', '棺椁'],
  '榔': ['槟榔', '榔头', '榔榆'],
  '椭': ['椭圆', '椭球', '椭圆形'],
  '粟': ['粟米', '粟子', '沧海一粟'],
  '棘': ['荆棘', '棘手', '棘刺'],
  '酣': ['酣睡', '酣畅', '酒酣'],
  '酥': ['酥软', '酥脆', '酥油'],
  '厦': ['大厦', '厦门', '广厦'],
  '硝': ['硝烟', '硝酸', '硝石'],
  '硫': ['硫酸', '硫磺', '硫化'],
  '雳': ['霹雳', '雳雳', '晴天霹雳'],
  '暂': ['暂时', '短暂', '暂停'],
  '敞': ['宽敞', '敞开', '敞亮'],
  '棠': ['海棠', '棠梨', '棠棣'],
  '睐': ['青睐', '明眸善睐', '睐盼'],
  '鼎': ['鼎盛', '鼎力', '鼎沸'],
  '喳': ['喳喳', '叽喳', '喊喳'],
  '喇': ['喇叭', '喇嘛', '哈喇'],
  '遏': ['遏制', '遏止', '怒不可遏'],
  '晾': ['晾晒', '晾干', '晾台'],
  '畴': ['范畴', '田畴', '平畴'],
  '跋': ['跋涉', '跋扈', '序跋'],
  '跛': ['跛脚', '跛行', '跛子'],
  '蜒': ['蜿蜒', '蜒蚰', '海蜒'],
  '蛤': ['蛤蜊', '蛤蟆', '蛤蚧'],
  '喻': ['比喻', '家喻户晓', '喻义'],
  '赌': ['赌博', '赌气', '赌咒'],
  '赎': ['赎罪', '赎金', '赎回'],
  '赐': ['赏赐', '赐予', '赐教'],
  '铸': ['铸造', '铸就', '铸铁'],
  '锈': ['生锈', '锈蚀', '铁锈'],
  '锌': ['锌片', '锌矿', '镀锌'],
  '甥': ['外甥', '甥女', '甥舅'],
  '掰': ['掰开', '掰手腕', '掰扯'],
  '氮': ['氮气', '氮肥', '氮氧'],
  '氯': ['氯气', '氯化', '氯仿'],
  '税': ['税收', '纳税', '税务'],
  '筐': ['筐子', '竹筐', '菜筐'],
  '筛': ['筛选', '筛子', '筛米'],
  '筏': ['竹筏', '木筏', '皮筏'],
  '储': ['储存', '储备', '储蓄'],
  '皓': ['皓月', '皓首', '皓齿'],
  '皖': ['皖南', '皖北', '皖江'],
  '粤': ['粤语', '粤菜', '粤剧'],
  '循': ['循环', '遵循', '循序渐进'],
  '逾': ['逾期', '逾越', '逾矩'],
  '腋': ['腋下', '腋窝', '腋毛'],
  '猩': ['猩猩', '猩红', '猩红色'],
  '猾': ['狡猾', '猾黠', '奸猾'],
  '惫': ['疲惫', '惫懒', '困惫'],
  '馈': ['馈赠', '反馈', '馈送'],
  '蛮': ['野蛮', '蛮横', '蛮荒'],
  '敦': ['敦厚', '敦煌', '敦促'],
  '斌': ['文质斌斌', '斌斌', '姓斌'],
  '痘': ['痘痘', '水痘', '牛痘'],
  '痢': ['痢疾', '痢菌', '赤痢'],
  '痪': ['瘫痪', '痪软', '偏痪'],
  '竣': ['竣工', '竣事', '告竣'],
  '翔': ['飞翔', '翱翔', '翔实'],
  '羡': ['羡慕', '艳羡', '歆羡'],
  '粪': ['粪便', '粪土', '粪肥'],
  '奠': ['奠定', '奠基', '祭奠'],
  '遂': ['遂心', '遂愿', '毛遂自荐'],
  '港': ['港口', '香港', '港湾'],
  '滞': ['停滞', '滞留', '滞销'],
  '湘': ['湘江', '湖南', '湘绣'],
  '渣': ['渣滓', '残渣', '豆渣'],
  '渤': ['渤海', '渤澥', '渤渤'],
  '渝': ['重庆', '渝州', '忠贞不渝'],
  '渲': ['渲染', '渲腾', '渲淡'],
  '愕': ['愕然', '惊愕', '错愕'],
  '愧': ['惭愧', '愧疚', '无愧'],
  '寓': ['公寓', '寓言', '寓意'],
  '窖': ['地窖', '酒窖', '菜窖'],
  '窘': ['窘迫', '窘态', '困窘'],
  '裕': ['富裕', '充裕', '裕如'],
  '禅': ['禅意', '禅师', '禅房'],
  '禄': ['俸禄', '福禄', '禄位'],
  '谣': ['谣言', '歌谣', '民谣'],
  '谤': ['诽谤', '谤议', '毁谤'],
  '犀': ['犀牛', '犀利', '犀角'],
  '屡': ['屡次', '屡屡', '屡见不鲜'],
  '隘': ['狭隘', '隘口', '关隘'],
  '媒': ['媒体', '媒介', '媒人'],
  '絮': ['柳絮', '棉絮', '絮叨'],
  '婿': ['女婿', '夫婿', '贤婿'],
  '缅': ['缅怀', '缅甸', '缅想'],
  '缆': ['电缆', '缆绳', '缆车'],
  '缉': ['缉拿', '缉毒', '通缉'],
  '缎': ['绸缎', '缎子', '锦缎'],
  '缔': ['缔结', '缔造', '取缔'],
  '骚': ['骚扰', '骚乱', '风骚'],
  '瑟': ['瑟瑟', '琴瑟', '萧瑟'],
  '鹉': ['鹦鹉', '鹉鹦', '鹦鹉学舌'],
  '瑰': ['玫瑰', '瑰丽', '瑰宝'],
  '瑙': ['玛瑙', '瑙鲁', '玛瑙石'],
  '携': ['携带', '携手', '提携'],
  '聘': ['聘请', '招聘', '聘礼'],
  '斟': ['斟酌', '斟酒', '斟茶'],
  '靶': ['靶子', '靶心', '打靶'],
  '墓': ['墓地', '墓碑', '扫墓'],
  '蓄': ['积蓄', '储蓄', '蓄水'],
  '椿': ['椿树', '椿萱', '香椿'],
  '楷': ['楷模', '楷书', '正楷'],
  '榄': ['橄榄', '榄仁', '榄角'],
  '槐': ['槐树', '槐花', '槐角'],
  '酪': ['奶酪', '酪蛋白', '乳酪'],
  '碍': ['妨碍', '障碍', '阻碍'],
  '碘': ['碘酒', '碘盐', '碘酊'],
  '碑': ['石碑', '碑文', '纪念碑'],
  '尴': ['尴尬', '尬尴', '尴尬症'],
  '辐': ['辐射', '辐条', '辐辏'],
  '辑': ['编辑', '辑录', '逻辑'],
  '频': ['频繁', '频道', '频率'],
  '鉴': ['鉴定', '借鉴', '鉴赏'],
  '睦': ['和睦', '睦邻', '敦睦'],
  '睫': ['睫毛', '眉睫', '迫在眉睫'],
  '睬': ['理睬', '睬理', '不睬'],
  '鄙': ['鄙视', '卑鄙', '鄙人'],
  '嗦': ['哆嗦', '嗦子', '啰嗦'],
  '盟': ['联盟', '盟友', '同盟'],
  '畸': ['畸形', '畸变', '畸零'],
  '跷': ['蹊跷', '跷跷板', '高跷'],
  '蜈': ['蜈蚣', '蜈虫', '蜈蚣草'],
  '蜗': ['蜗牛', '蜗居', '蜗轮'],
  '蛾': ['飞蛾', '蛾子', '蛾眉'],
  '蜕': ['蜕变', '蜕皮', '蜕化'],
  '署': ['署名', '部署', '公署'],
  '蜀': ['蜀国', '蜀锦', '蜀道'],
  '幌': ['幌子', '幌动', '虚幌'],
  '锚': ['船锚', '锚点', '抛锚'],
  '锡': ['锡纸', '锡矿', '无锡'],
  '锣': ['锣鼓', '铜锣', '敲锣'],
  '锥': ['锥子', '圆锥', '锥度'],
  '锯': ['锯子', '锯木', '电锯'],
  '锰': ['锰矿', '锰钢', '二氧化锰'],
  '魁': ['魁梧', '魁首', '花魁'],
  '衙': ['衙门', '衙役', '县衙'],
  '腥': ['腥味', '血腥', '腥气'],
  '腺': ['腺体', '乳腺', '甲状腺'],
  '鹏': ['大鹏', '鹏程', '鲲鹏'],
  '鲍': ['鲍鱼', '鲍叔牙', '姓鲍'],
  '猿': ['猿猴', '猿人', '心猿意马'],
  '颖': ['新颖', '聪颖', '颖悟'],
  '煞': ['煞费苦心', '煞白', '凶神恶煞'],
  '雏': ['雏鸟', '雏形', '雏菊'],
  '馍': ['馍馍', '肉夹馍', '蒸馍'],
  '馏': ['蒸馏', '馏分', '干馏'],
  '禀': ['禀赋', '禀报', '禀性'],
  '痹': ['麻痹', '痹症', '风痹'],
  '廓': ['轮廓', '廓清', '耳廓'],
  '痰': ['吐痰', '痰盂', '化痰'],
  '廉': ['廉洁', '廉价', '清廉'],
  '靖': ['靖边', '靖康', '绥靖'],
  '誊': ['誊写', '誊清', '誊录'],
  '煎': ['煎熬', '煎饼', '煎药'],
  '滇': ['滇池', '滇红', '滇剧'],
  '滤': ['过滤', '滤纸', '滤液'],
  '滔': ['滔滔', '滔天', '滔荡'],
  '漓': ['淋漓', '漓江', '漓漓'],
  '溯': ['追溯', '溯源', '回溯'],
  '溶': ['溶解', '溶液', '溶化'],
  '粱': ['高粱', '粱米', '黄粱一梦'],
  '寝': ['寝室', '就寝', '安寝'],
  '谨': ['谨慎', '严谨', '谨记'],
  '褂': ['褂子', '马褂', '短褂'],
  '谬': ['荒谬', '谬误', '谬论'],
  '嫉': ['嫉妒', '嫉恨', '嫉恶如仇'],
  '嫁': ['出嫁', '嫁接', '嫁妆'],
  '缤': ['缤纷', '缤翻', '五彩缤纷'],
  '剿': ['围剿', '剿灭', '剿匪'],
  '赘': ['累赘', '赘言', '入赘'],
  '墟': ['废墟', '墟落', '殷墟'],
  '嘉': ['嘉奖', '嘉宾', '嘉许'],
  '摧': ['摧毁', '摧残', '摧折'],
  '赫': ['显赫', '赫然', '赫兹'],
  '撇': ['撇开', '撇嘴', '撇弃'],
  '摹': ['临摹', '摹仿', '摹写'],
  '蔓': ['蔓延', '蔓草', '枝蔓'],
  '蔑': ['蔑视', '污蔑', '轻蔑'],
  '蔡': ['蔡伦', '姓蔡', '蔡国'],
  '熙': ['熙熙攘攘', '康熙', '熙和'],
  '蔚': ['蔚蓝', '蔚然', '蔚为大观'],
  '兢': ['兢兢业业', '兢慎', '战战兢兢'],
  '酵': ['发酵', '酵母', '酵子'],
  '酿': ['酿造', '酿酒', '酝酿'],
  '碱': ['碱性', '碱面', '纯碱'],
  '辖': ['管辖', '辖区', '直辖'],
  '辗': ['辗转', '辗动', '辗转反侧'],
  '雌': ['雌性', '雌雄', '雌花'],
  '墅': ['别墅', '野墅', '田墅'],
  '踊': ['踊跃', '踊动', '踊贵'],
  '嘀': ['嘀咕', '嘀嗒', '嘀哩'],
  '锹': ['铁锹', '锹镐', '锹土'],
  '镀': ['镀金', '电镀', '镀层'],
  '箕': ['簸箕', '箕斗', '箕踞'],
  '箫': ['箫笛', '洞箫', '吹箫'],
  '舆': ['舆论', '舆图', '车舆'],
  '僚': ['官僚', '同僚', '幕僚'],
  '僧': ['僧人', '僧侣', '高僧'],
  '魄': ['气魄', '魂魄', '体魄'],
  '魅': ['魅力', '鬼魅', '魅惑'],
  '膜': ['薄膜', '膜拜', '隔膜'],
  '馒': ['馒头', '馒首', '蒸馒头'],
  '膏': ['牙膏', '膏药', '膏腴'],
  '瘟': ['瘟疫', '瘟神', '瘟鸡'],
  '彰': ['表彰', '彰显', '昭彰'],
  '粹': ['纯粹', '精粹', '国粹'],
  '歉': ['道歉', '歉意', '歉收'],
  '弊': ['弊端', '作弊', '利弊'],
  '熔': ['熔化', '熔炉', '熔岩'],
  '煽': ['煽动', '煽情', '煽风点火'],
  '漱': ['漱口', '漱洗', '漱盂'],
  '寡': ['寡妇', '寡言', '寡不敌众'],
  '寥': ['寥寥', '寥廓', '寂寥'],
  '谭': ['姓谭', '谭嗣同', '天方夜谭'],
  '肇': ['肇事', '肇始', '肇端'],
  '褪': ['褪色', '褪毛', '褪套'],
  '骡': ['骡子', '骡马', '驴骡'],
  '撩': ['撩拨', '撩起', '撩动'],
  '撮': ['撮合', '撮要', '一撮'],
  '撬': ['撬开', '撬锁', '撬棍'],
  '擒': ['擒拿', '擒获', '欲擒故纵'],
  '撰': ['撰写', '撰述', '撰稿'],
  '鞍': ['马鞍', '鞍马', '鞍鞯'],
  '蕴': ['蕴藏', '蕴含', '底蕴'],
  '槽': ['水槽', '槽头', '槽钢'],
  '樟': ['樟树', '樟脑', '樟木'],
  '橄': ['橄榄', '橄榄油', '橄榄枝'],
  '敷': ['敷衍', '敷设', '热敷'],
  '醇': ['醇厚', '醇酒', '醇香'],
  '磕': ['磕头', '磕碰', '磕绊'],
  '磊': ['磊落', '磊磊', '光明磊落'],
  '磅': ['磅礴', '磅秤', '过磅'],
  '碾': ['碾压', '碾米', '碾子'],
  '霄': ['云霄', '霄汉', '九霄'],
  '瞒': ['隐瞒', '瞒骗', '欺瞒'],
  '嘲': ['嘲笑', '嘲讽', '嘲弄'],
  '嘹': ['嘹亮', '嘹唳', '嘹嘹'],
  '蝎': ['蝎子', '蝎毒', '蛇蝎'],
  '蝌': ['蝌蚪', '蝌子', '蝌蚪文'],
  '蝗': ['蝗虫', '蝗灾', '灭蝗'],
  '镐': ['镐头', '铁镐', '洋镐'],
  '镑': ['英镑', '镑亏', '金镑'],
  '稽': ['稽查', '稽核', '滑稽'],
  '篓': ['篓子', '竹篓', '背篓'],
  '鲤': ['鲤鱼', '鲤跃', '锦鲤'],
  '鲫': ['鲫鱼', '鲫瓜子', '金鲫'],
  '褒': ['褒奖', '褒义', '褒贬'],
  '瘪': ['干瘪', '瘪三', '瘪嘴'],
  '瘤': ['肿瘤', '瘤子', '毒瘤'],
  '瘫': ['瘫痪', '瘫软', '瘫倒'],
  '遵': ['遵守', '遵循', '遵从'],
  '憋': ['憋气', '憋闷', '憋屈'],
  '潭': ['水潭', '深潭', '泥潭'],
  '鲨': ['鲨鱼', '鲨鳍', '虎鲨'],
  '澳': ['澳门', '澳洲', '港澳'],
  '潘': ['姓潘', '潘安', '潘多拉'],
  '澈': ['清澈', '澈底', '明澈'],
  '澜': ['波澜', '澜漪', '推波助澜'],
  '憔': ['憔悴', '憔瘁', '憔虑'],
  '懊': ['懊恼', '懊悔', '懊丧'],
  '翩': ['翩翩', '翩然', '翩跹'],
  '褥': ['被褥', '褥子', '褥疮'],
  '谴': ['谴责', '谴谪', '谴告'],
  '憨': ['憨厚', '憨直', '憨笑'],
  '豫': ['犹豫', '豫剧', '河南'],
  '缭': ['缭绕', '缭乱', '缭乱'],
  '撼': ['震撼', '撼动', '摇撼'],
  '擅': ['擅长', '擅自', '擅离职守'],
  '薯': ['红薯', '马铃薯', '薯片'],
  '薛': ['姓薛', '薛仁贵', '薛宝钗'],
  '薇': ['蔷薇', '薇菜', '紫薇'],
  '擎': ['擎起', '擎天柱', '引擎'],
  '薪': ['薪水', '薪柴', '加薪'],
  '颠': ['颠倒', '颠覆', '颠簸'],
  '翰': ['翰林', '翰墨', '文翰'],
  '噩': ['噩梦', '噩耗', '浑浑噩噩'],
  '橱': ['橱柜', '橱窗', '衣橱'],
  '霍': ['霍然', '霍乱', '姓霍'],
  '辙': ['车辙', '辙痕', '南辕北辙'],
  '冀': ['希冀', '冀望', '河北'],
  '踱': ['踱步', '踱来踱去', '踱方步'],
  '蹂': ['蹂躏', '蹂踏', '蹂践'],
  '蟆': ['蛤蟆', '蟆蝈', '癞蛤蟆'],
  '鹦': ['鹦鹉', '鹦哥', '鹦鹉学舌'],
  '黔': ['黔驴技穷', '黔首', '贵州'],
  '穆': ['肃穆', '穆然', '姓穆'],
  '篡': ['篡改', '篡夺', '篡位'],
  '儒': ['儒家', '儒雅', '儒生'],
  '邀': ['邀请', '邀约', '邀功'],
  '鲸': ['鲸鱼', '鲸吞', '蓝鲸'],
  '瘾': ['上瘾', '瘾君子', '烟瘾'],
  '瘸': ['瘸腿', '瘸子', '一瘸一拐'],
  '糙': ['粗糙', '糙米', '糙纸'],
  '濒': ['濒临', '濒死', '濒危'],
  '憾': ['遗憾', '憾事', '缺憾'],
  '懈': ['松懈', '懈怠', '不懈'],
  '缰': ['缰绳', '缰锁', '脱缰'],
  '缴': ['缴纳', '缴获', '上缴'],
  '鞠': ['鞠躬', '鞠养', '蹴鞠'],
  '藐': ['藐视', '藐小', '藐忽'],
  '檀': ['檀香', '檀木', '紫檀'],
  '礁': ['礁石', '暗礁', '珊瑚礁'],
  '磷': ['磷肥', '磷火', '磷酸'],
  '瞭': ['瞭望', '瞭哨', '瞭见'],
  '瞳': ['瞳孔', '瞳仁', '双瞳'],
  '瞩': ['瞩目', '瞩望', '高瞻远瞩'],
  '曙': ['曙光', '曙色', '曙天'],
  '蹋': ['糟蹋', '蹋顿', '蹴蹋'],
  '嚎': ['嚎叫', '嚎哭', '哀嚎'],
  '赡': ['赡养', '赡家', '丰赡'],
  '魏': ['魏国', '北魏', '姓魏'],
  '簧': ['弹簧', '簧片', '双簧'],
  '徽': ['徽章', '安徽', '国徽'],
  '爵': ['爵位', '爵士', '伯爵'],
  '臊': ['臊气', '臊子', '害臊'],
  '鳄': ['鳄鱼', '鳄梨', '扬子鳄'],
  '癌': ['癌症', '癌变', '抗癌'],
  '糠': ['米糠', '糠皮', '糟糠'],
  '豁': ['豁达', '豁口', '豁然开朗'],
  '藕': ['莲藕', '藕粉', '藕断丝连'],
  '瞻': ['瞻仰', '瞻望', '高瞻远瞩'],
  '嚣': ['嚣张', '喧嚣', '叫嚣'],
  '镰': ['镰刀', '镰仓', '开镰'],
  '鳍': ['鱼鳍', '鳍条', '背鳍'],
  '璧': ['璧玉', '璧还', '完璧归赵'],
  '戳': ['戳穿', '戳记', '邮戳'],
  '孽': ['罪孽', '孽障', '妖孽'],
  '藻': ['海藻', '水藻', '辞藻'],
  '蹬': ['蹬腿', '蹬车', '蹬踏'],
  '巅': ['山巅', '巅峰', '巅崖'],
  '簸': ['颠簸', '簸箕', '簸动'],
  '簿': ['账簿', '簿册', '练习簿'],
  '靡': ['风靡', '靡靡', '萎靡'],
  '癣': ['癣疥', '牛皮癣', '手癣'],
  '羹': ['羹汤', '残羹', '闭门羹'],
  '鳖': ['鳖甲', '土鳖', '瓮中之鳖'],
  '鬓': ['鬓角', '鬓发', '双鬓'],
  '壤': ['土壤', '接壤', '天壤之别'],
  '馨': ['温馨', '馨香', '馨逸'],
  '躁': ['急躁', '烦躁', '浮躁'],
  '蠕': ['蠕动', '蠕蠕', '蠕虫'],
  '鳞': ['鱼鳞', '鳞片', '遍体鳞伤'],
  '糯': ['糯米', '糯稻', '软糯'],
  '霹': ['霹雳', '霹雷', '晴天霹雳'],
  '躏': ['蹂躏', '躏践', '躏藉'],
  '黯': ['黯然', '黯淡', '黯黑'],
  '髓': ['骨髓', '精髓', '脑髓'],
  '赣': ['江西', '赣江', '赣南'],
  '瓤': ['瓜瓤', '瓤子', '橘瓤'],
  '矗': ['矗立', '矗然', '矗矗'],
};

/**
 * 造句：从句子库中提取包含该字的儿童句子。
 * @param {string} hanzi
 * @param {number} count
 * @returns {Array<string>}
 */
function getSentences(hanzi, count = 2) {
  const sentences = loadSentences();
  const candidates = sentences.filter((s) => s.text && s.text.includes(hanzi));
  return candidates.slice(0, count).map((s) => s.text);
}

/**
 * 字义解释：优先使用新华字典，其次用内置高频字解释，最后用通用模板。
 * @param {string} hanzi
 * @param {string} pinyin
 * @returns {string}
 */
function getMeaning(hanzi, pinyin) {
  const xinhua = loadXinhua();

  // 0. 高频虚词直接用内置解释（新华字典的古义不适合儿童）
  const functionWords = ['的', '了', '是', '在', '有', '和', '就', '不', '也', '都', '还', '又', '再', '才', '却', '便', '与', '及', '或', '而', '且', '但', '然', '则', '乃', '即', '若', '如', '因', '为', '所', '以', '之', '乎', '者', '也', '矣', '焉', '哉', '吗', '呢', '吧', '啊', '呀', '哇', '哦', '唉', '嗯'];
  if (functionWords.includes(hanzi) && commonMeanings[hanzi]) {
    return commonMeanings[hanzi];
  }

  // 1. 优先使用新华字典解释
  if (xinhua[hanzi] && xinhua[hanzi].meaning) {
    let meaning = xinhua[hanzi].meaning;

    // 处理字典用法说明：如"〔～蟹〕见蟹"。" → "螃蟹的螃，和蟹组成'螃蟹'"
    const usageMatch = meaning.match(/〔～(.+?)〕见(.+?)[”"]?[。.]?/);
    if (usageMatch) {
      const word = hanzi + usageMatch[1];
      meaning = `${word}的${hanzi}，和${usageMatch[1]}组成「${word}」`;
    }

    // 过滤包含英文解释的内容（如"[butterfly]。如蝶子..."）
    if (meaning.includes('[') && meaning.includes(']')) {
      const bracketMatch = meaning.match(/\[([^\]]+)\]/);
      if (bracketMatch) {
        // 提取英文对应的中文解释，或者用内置解释
        if (commonMeanings[hanzi]) {
          meaning = commonMeanings[hanzi];
        } else {
          meaning = meaning.replace(/\[[^\]]*\]/g, '').replace(/[。.]如.+/, '').trim();
        }
      }
    }

    // 过滤"蜻〈名〉的蜻"这种解释
    if (/[〈<][名动形数量代副介连助叹拟声][〉>]/.test(meaning) && meaning.length < 15) {
      if (commonMeanings[hanzi]) {
        meaning = commonMeanings[hanzi];
      } else {
        meaning = meaning.replace(/[〈<][名动形数量代副介连助叹拟声][〉>]/g, '').replace(/的[\u4e00-\u9fa5]$/, '').trim();
      }
    }

    // 检测是否为古义（不适合儿童的解释）
    const isOldMeaning = /(古|本义|古代|正梁|水名|山名|地名|姓氏|通假|假借|本指|原指|起身|腰带|束衣|刑法|法度|甲骨文|金文|小篆|部首|貌美|《|》|郑笺|曰|云|①|②|③|④|⑤)/.test(meaning)
      || meaning.length < 4
      || meaning.length > 50
      || meaning.endsWith('的')
      || /^[a-z\s]+$/i.test(meaning) // 只有拼音没有解释
      || (meaning.includes(hanzi + ' ') && meaning.length < 10) // 只有字+拼音
      || /^[\u4e00-\u9fa5][a-zāáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜ\s（）()]+$/i.test(meaning) // 只有字+拼音
      || /^[\u4e00-\u9fa5]{1,4}的[\u4e00-\u9fa5]$/.test(meaning); // "美,貌美的好"这种格式

    if (isOldMeaning && commonMeanings[hanzi]) {
      meaning = commonMeanings[hanzi];
    } else if (meaning.length < 6 && commonMeanings[hanzi]) {
      meaning = commonMeanings[hanzi];
    }

    // 最后兜底：如果解释为空、只有一个字、或是字典用法说明，用组词构造解释
    if (!meaning || meaning.length <= 1
      || /^见/.test(meaning) // "见［蜻蜓]"、"见柠檬"的檬"
      || /^如/.test(meaning) // "如葡萄"的萄"
      || /（[^\u4e00-\u9fa5]+）/.test(meaning) // 包含繁体字括号
      || /^[\u4e00-\u9fa5]（/.test(meaning) // "柠（檸）níng"
    ) {
      const words = getWords(hanzi, 1);
      if (words.length > 0) {
        const word = words[0].word;
        // 找到该字在词中的位置，构造解释
        const idx = word.indexOf(hanzi);
        if (idx >= 0 && word.length >= 2) {
          meaning = `${word}的${hanzi}，和其他字组成「${word}」`;
        } else {
          meaning = `这个字读「${hanzi}」，常见于「${word}」一词`;
        }
      } else {
        meaning = `这个字读「${hanzi}」，是一个常用汉字。`;
      }
    }

    return meaning;
  }

  // 2. 内置高频字解释
  if (commonMeanings[hanzi]) return commonMeanings[hanzi];

  // 3. 通用模板
  const py = pinyin || '';
  return `这个字读「${py || hanzi}」，是一个常用汉字。`;
}

// 内置高频字解释（100字），用于新华字典解释太短或不准确时补充
const commonMeanings = {
    '一': '数字，表示最小的正整数',
    '二': '数字，一加一的和',
    '三': '数字，二加一的和',
    '四': '数字，三加一的和',
    '五': '数字，四加一的和',
    '六': '数字，五加一的和',
    '七': '数字，六加一的和',
    '八': '数字，七加一的和',
    '九': '数字，八加一的和',
    '十': '数字，九加一的和',
    '人': '能制造工具并使用工具进行劳动的高等动物',
    '口': '嘴，人和动物吃东西和发声的器官',
    '手': '人体上肢前端能拿东西的部分',
    '目': '眼睛',
    '耳': '听觉器官',
    '日': '太阳；白天',
    '月': '月亮；计时单位',
    '水': '无色无味的液体，生命之源',
    '火': '物体燃烧时发出的光和热',
    '山': '地面上由土石构成的高耸部分',
    '石': '构成地壳的矿物质硬块',
    '田': '种植农作物的土地',
    '禾': '谷类植物的统称',
    '虫': '昆虫和类似昆虫的小动物',
    '云': '天空中悬浮的水滴或冰晶聚集物',
    '雨': '从云层中降向地面的水',
    '雪': '天空中飘落的白色结晶',
    '风': '空气流动的现象',
    '花': '植物的繁殖器官，有各种颜色和形状',
    '鸟': '脊椎动物的一纲，有羽毛，会飞',
    '鱼': '生活在水中的脊椎动物，用鳃呼吸',
    '马': '哺乳动物，四肢强健，善跑',
    '牛': '哺乳动物，头上有角，能耕地产奶',
    '羊': '哺乳动物，头上有角，毛可做衣服',
    '大': '在体积、面积、数量等方面超过一般',
    '小': '在体积、面积、数量等方面不及一般',
    '上': '位置在高处的',
    '下': '位置在低处的',
    '多': '数量大',
    '少': '数量小',
    '来': '从别的地方到说话人所在的地方',
    '去': '从所在地到别的地方',
    '开': '使关闭着的东西不再关闭',
    '关': '使开着的物体合拢',
    '好': '优点多的，使人满意的',
    '坏': '品质恶劣，有害的',
    '我': '自称，自己',
    '你': '称对方',
    '他': '称自己和对方以外的男性',
    '她': '称自己和对方以外的女性',
    '它': '称人以外的事物',
    '们': '用在代词或指人的名词后面表示复数',
    '爸': '父亲',
    '妈': '母亲',
    '哥': '哥哥，同父母或同辈而年纪比自己大的男子',
    '弟': '弟弟，同父母或同辈而年纪比自己小的男子',
    '姐': '姐姐，同父母或同辈而年纪比自己大的女子',
    '妹': '妹妹，同父母或同辈而年纪比自己小的女子',
    '爷': '祖父',
    '奶': '祖母',
    '学': '学习，效法',
    '校': '学校，教育机构',
    '老': '年纪大的',
    '师': '老师，传授知识的人',
    '生': '出生，生长',
    '读': '看着文字念出声',
    '写': '用笔在纸上或其他东西上做字',
    '说': '用话来表达意思',
    '听': '用耳朵接受声音',
    '看': '使视线接触人或物',
    '问': '有不知道或不明白的事请人解答',
    '想': '动脑筋，思索',
    '做': '进行工作或活动',
    '玩': '游戏，玩耍',
    '笑': '露出愉快的表情，发出欢喜的声音',
    '哭': '因痛苦或悲哀而流泪发声',
    '吃': '把食物等放到嘴里经过咀嚼咽下去',
    '喝': '把液体或流食咽下去',
    '睡': '睡觉，休息',
    '走': '人或鸟兽的脚交互向前移动',
    '跑': '两只脚或四条腿迅速前进',
    '跳': '两脚用力离开地面向上或向前',
    '飞': '鸟、虫等在空中活动',
    '游': '人或动物在水里行动',
    '坐': '把臀部放在椅子等物体上支持身体',
    '站': '直着身体，两脚着地',
    '是': '表示判断、肯定',
    '不': '表示否定',
    '有': '表示存在',
    '无': '没有',
    '的': '用在定语后面，表示修饰关系',
    '在': '表示人或事物的位置',
    '和': '表示并列关系',
    '了': '表示动作已经完成',
    '就': '表示承接、肯定',
    '不': '表示否定',
    '也': '表示同样、并行',
    '都': '表示全部',
    '还': '表示仍然、继续',
    '又': '表示重复、继续',
    '才': '表示刚刚、仅仅',
    '却': '表示转折',
    '便': '表示就、即',
    '着': '表示动作正在进行',
    '过': '表示动作曾经发生',
    // 补充常见字的现代常用义（新华字典本义是古义的字）
    '极': '顶点，尽头；非常',
    '洋': '海洋，广大；外国的',
    '作': '做，进行；作品',
    '给': '交付，送与；为，替',
    '带': '带领，携带；带子',
    '法': '方法，办法；法律',
    '傍': '靠近，临近',
    '坏': '品质恶劣，有害的；损坏',
    '眼': '眼睛，眼珠',
    '睛': '眼珠，眼睛',
    '宽': '宽阔，宽大；放宽',
    '找': '寻找，寻求',
    '变': '变化，改变',
    '海': '大海，海洋',
    '穆': '严肃，恭敬',
    '肃': '严肃，恭敬',
    '静': '安静，没有声音',
    '安': '安全，平安；安排',
  };

/**
 * 造句：四级降级策略（按适合儿童的程度排序）
 * 1. 优先从儿童句子库（sentences.json）提取
 * 2. 其次用组词模板造句（基于常用词生成简单句子）
 * 3. 最后从新华字典例句中提取（兜底，筛选简单的）
 * 4. 都没有则返回空
 * @param {string} hanzi
 * @param {number} count
 * @returns {Array<string>}
 */
function getSentences(hanzi, count = 2) {
  const sentences = loadSentences();
  const sentencesByChar = loadSentencesByChar();
  const xinhua = loadXinhua();

  // 0. 优先使用手工造句库（质量最高）
  if (sentencesByChar[hanzi] && sentencesByChar[hanzi].length > 0) {
    return sentencesByChar[hanzi].slice(0, count);
  }

  // 1. 从儿童句子库提取
  const kidSentences = sentences
    .filter((s) => s.text && s.text.includes(hanzi))
    .map((s) => s.text);

  if (kidSentences.length > 0) {
    return kidSentences.slice(0, count);
  }

  // 2. 用组词模板造句（比新华字典古文更适合儿童）
  const templateSentence = makeTemplateSentence(hanzi);
  if (templateSentence) {
    return [templateSentence];
  }

  // 3. 从新华字典例句中提取（兜底，严格筛选）
  if (xinhua[hanzi] && xinhua[hanzi].examples) {
    const simpleExamples = xinhua[hanzi].examples.filter((s) => {
      if (s.length > 20 || s.length < 5) return false;
      // 过滤古文
      if (/(矣|乎|焉|哉|也|兮|欤|亦|之乎者也)/.test(s)) return false;
      // 过滤包含拼音、数字、括号解释的行
      if (/[a-zāáǎàēéěèīíǐìōóǒòūúǔù]/.test(s)) return false;
      if (/\d/.test(s)) return false;
      if (s.includes('[') || s.includes(']') || s.includes('〔') || s.includes('〕')) return false;
      // 过滤不适合儿童的词
      if (/(霹雳|崩|歹徒|碴|刑法|酒浸|治风)/.test(s)) return false;
      return true;
    });
    if (simpleExamples.length > 0) {
      return simpleExamples.slice(0, count);
    }
  }

  return [];
}

/**
 * 用组词模板造句：基于常用词生成简单、适合儿童的句子。
 * @param {string} hanzi
 * @returns {string|null}
 */
function makeTemplateSentence(hanzi) {
  const words = getWords(hanzi, 3);
  if (words.length === 0) return null;

  // 取第一个常用词
  const word = words[0].word;
  const xinhua = loadXinhua();
  const pos = xinhua[hanzi] && xinhua[hanzi].pos ? xinhua[hanzi].pos[0] : '';

  // 动物/食物类名词模板
  const animalWords = ['螃蟹', '蝴蝶', '蜻蜓', '蚂蚁', '蜜蜂', '小鸟', '小鱼', '兔子', '老虎', '狮子', '大象', '猴子', '熊猫', '青蛙', '虫子', '鹦鹉', '鲸鱼', '海豚', '企鹅', '长颈鹿'];
  const foodWords = ['苹果', '香蕉', '西瓜', '葡萄', '草莓', '蛋糕', '饼干', '糖果', '米饭', '面条', '饺子', '包子', '牛奶', '果汁', '柠檬', '巧克力', '咖啡', '橙子', '桃子', '梨子'];

  if (animalWords.includes(word)) {
    return `${word}真可爱`;
  }
  if (foodWords.includes(word)) {
    return `我喜欢吃${word}`;
  }

  // 常见形容词列表（用"很XX"模板）
  const adjectiveWords = ['肃穆', '安静', '美丽', '漂亮', '高兴', '快乐', '开心', '难过', '伤心', '害怕', '勇敢', '聪明', '勤劳', '善良', '诚实', '认真', '仔细', '马虎', '骄傲', '谦虚', '热情', '冷淡', '温柔', '粗暴', '勤劳', '懒惰', '干净', '肮脏', '整齐', '混乱', '明亮', '黑暗', '温暖', '寒冷', '炎热', '凉爽', '潮湿', '干燥', '宽阔', '狭窄', '高大', '矮小', '肥胖', '瘦弱', '美丽', '丑陋', '善良', '凶恶', '聪明', '愚蠢', '勇敢', '胆小', '勤劳', '懒惰', '认真', '马虎', '仔细', '粗心', '谦虚', '骄傲', '诚实', '虚伪', '热情', '冷淡', '温柔', '粗暴'];

  if (adjectiveWords.includes(word) || pos === '形') {
    return `这里很${word}`;
  }

  // 常见动词列表（用"我喜欢XX"模板）
  const verbWords = ['学习', '读书', '写字', '画画', '唱歌', '跳舞', '跑步', '游泳', '踢球', '爬山', '旅行', '游戏', '玩耍', '睡觉', '吃饭', '喝水', '洗澡', '刷牙', '穿衣', '走路'];

  if (verbWords.includes(word) || pos === '动') {
    return `我喜欢${word}`;
  }

  // 常见名词（具体事物，可用"我看到了XX"）
  const concreteNouns = ['学校', '老师', '同学', '教室', '书本', '铅笔', '橡皮', '尺子', '书包', '桌子', '椅子', '窗户', '门', '房子', '汽车', '自行车', '飞机', '火车', '船', '树', '花', '草', '太阳', '月亮', '星星', '云', '雨', '雪', '风', '山', '水', '河', '海', '湖', '猫', '狗', '鸟', '鱼', '兔子', '老虎', '狮子', '大象', '猴子', '熊猫', '苹果', '香蕉', '西瓜', '葡萄', '草莓', '蛋糕', '饼干', '糖果', '米饭', '面条', '饺子', '包子', '牛奶', '果汁', '柠檬', '巧克力', '咖啡', '螃蟹', '蝴蝶', '蜻蜓', '蚂蚁', '蜜蜂', '鹦鹉', '企鹅', '长颈鹿', '海豚', '鲸鱼'];

  // 抽象名词（用"我知道XX"更合适）
  const abstractNouns = ['薪水', '年薪', '工资', '知识', '友谊', '爱情', '亲情', '时间', '金钱', '健康', '快乐', '幸福', '勇敢', '聪明', '勤劳', '善良', '诚实', '认真', '仔细', '谦虚', '骄傲', '热情', '温柔', '美丽', '漂亮', '高兴', '难过', '伤心', '害怕', '安静', '肃穆', '严肃', '平静', '冷静', '安全', '危险', '困难', '容易', '简单', '复杂', '重要', '必要', '可能', '应该', '可以', '能够', '需要', '想要', '希望', '梦想', '理想', '目标', '计划', '安排', '工作', '学习', '生活', '家庭', '社会', '国家', '世界', '历史', '文化', '艺术', '科学', '技术', '数学', '语文', '英语', '体育', '音乐', '美术', '游戏', '运动', '旅行', '假期', '周末', '节日', '生日', '礼物', '惊喜'];

  if (concreteNouns.includes(word)) {
    return `我看到了${word}`;
  }
  if (abstractNouns.includes(word)) {
    return `我知道${word}`;
  }

  // 名词默认用"我知道XX"（比"我看到了XX"更通用，避免抽象名词不自然）
  if (pos === '名') {
    return `我知道${word}`;
  }

  // 通用模板：用"我知道XX"比"XX真有意思"更通顺
  return `我知道${word}`;
}

/**
 * 句子解读：解释整句话的意思，用儿童能理解的语言。
 * @param {string} hanzi
 * @param {string} sentence
 * @param {string} [pos] 词性
 * @param {string} [meaning] 字义
 * @returns {string}
 */
function getUsage(hanzi, sentence, pos, meaning) {
  if (!sentence) return '';

  // 模板句子的解读（根据模板类型准确生成）
  if (sentence.startsWith('我知道')) {
    const word = sentence.replace('我知道', '');
    return `「${sentence}」是说我了解${word}是什么。`;
  }
  if (sentence.startsWith('我喜欢吃')) {
    const word = sentence.replace('我喜欢吃', '');
    return `「${sentence}」是说我觉得${word}很好吃。`;
  }
  if (sentence.endsWith('真可爱')) {
    const word = sentence.replace('真可爱', '');
    return `「${sentence}」是说${word}很讨人喜欢。`;
  }
  if (sentence.startsWith('这里很')) {
    const word = sentence.replace('这里很', '');
    return `「${sentence}」是说这个地方很${word}。`;
  }
  if (sentence.startsWith('我喜欢') && !sentence.startsWith('我喜欢吃')) {
    const word = sentence.replace('我喜欢', '');
    return `「${sentence}」是说我很喜欢${word}。`;
  }
  if (sentence.startsWith('我看到了')) {
    const word = sentence.replace('我看到了', '');
    return `「${sentence}」是说我看见了${word}。`;
  }

  // 儿童句子库句子：通用解读（句子本身简单，直接说明句意）
  // 不强行替换字义，避免出现"我是小学习，效法生"这种不通顺的情况
  // 去掉末尾句号避免双句号
  const cleanSentence = sentence.replace(/[。.!！?？]+$/, '');
  return `「${cleanSentence}」描述的是：${cleanSentence}。`;
}

/**
 * 生成完整的教学内容。
 * @param {Object} char { hanzi, pinyin }
 * @param {Object} [context] { currentSentence } 当前学习的句子（用于用法说明）
 * @returns {Promise<Object>} { hanzi, pinyin, words, meaning, sentences, usage, speakParts }
 */
async function teach(char, context = {}) {
  const hanzi = char.hanzi;
  const xinhua = loadXinhua();
  const words = getWords(hanzi, 3);
  const sentences = getSentences(hanzi, 2);

  // 拼音：优先新华字典拼音，其次字库拼音；多音字根据实际组词动态匹配读音
  let pinyin = (xinhua[hanzi] && xinhua[hanzi].pinyin) || char.pinyin || '';
  const dynPinyin = resolvePolyphone(hanzi, words);
  if (dynPinyin) pinyin = dynPinyin;

  // currentSentence 只有在是完整句子（长度>1且不等于单字）时才用作造句
  // 避免单字格学习时 currentSentence=单字，导致造句只有一个字
  let currentSentence = context.currentSentence || '';
  if (currentSentence.length <= 1 || currentSentence === hanzi) {
    currentSentence = '';
  }
  const exampleSentence = currentSentence || sentences[0] || '';

  const pos = xinhua[hanzi] && xinhua[hanzi].pos ? xinhua[hanzi].pos : '';

  let meaning = getMeaning(hanzi, pinyin);
  let usage = getUsage(hanzi, exampleSentence, pos, meaning);

  // AI 接口：如果注入了 AI 教学生成器，用 AI 生成更准确的解释和用法
  if (aiTeacher) {
    try {
      const aiResult = await aiTeacher.generate(hanzi, { pinyin, words, sentences, currentSentence: exampleSentence, pos });
      if (aiResult && aiResult.meaning) meaning = aiResult.meaning;
      if (aiResult && aiResult.usage) usage = aiResult.usage;
    } catch (e) {
      // AI 失败，用模板结果
    }
  }

  // 语音朗读：组成一个完整的句子
  // 格式：这个字念X，组词1的X，组词2的X，组词3的X，造句。
  let speakText = `这个字念${hanzi}`;
  if (words.length > 0) {
    const wordParts = words.slice(0, 3).map(w => `${w.word}的${hanzi}`);
    speakText += '，' + wordParts.join('，');
  }
  if (exampleSentence) {
    speakText += '，' + exampleSentence;
  }
  // 确保以句号结尾，但避免双句号
  if (!speakText.endsWith('。')) {
    speakText += '。';
  }

  const speakParts = [speakText];

  return {
    hanzi,
    pinyin,
    words,
    meaning,
    sentences,
    usage,
    exampleSentence,
    pos,
    speakParts,
  };
}

/**
 * 注入 AI 教学内容生成器（预留）。
 * @param {{generate: Function}} teacher
 */
function setAITeacher(teacher) {
  if (teacher && typeof teacher.generate === 'function') {
    aiTeacher = teacher;
  }
}

module.exports = {
  init,
  teach,
  getWords,
  getSentences,
  getMeaning,
  getUsage,
  setAITeacher,
  resolvePolyphone,
};

/**
 * 多音字读音表：汉字 → { 读音: [包含该读音的组词] }
 * 用于教学时根据实际组词匹配正确读音（字典首音不一定是教学常用音）。
 */
const POLYPHONE = {
  '瀑': { 'pù': ['瀑布', '飞瀑', '瀑流'], 'bào': ['瀑河'] },
  '什': { 'shén': ['什么', '干什么', '为什么'], 'shí': ['家什', '什锦'] },
  '宿': { 'sù': ['宿舍', '归宿', '住宿', '寄宿', '食宿', '宿营', '宿主'], 'xiǔ': ['一宿', '两宿'] },
  '干': { 'gàn': ['干部', '骨干', '干事', '干吗', '干活'], 'gān': ['干净', '若干', '干涉', '干旱', '干脆', '干扰', '干预', '饼干', '风干'] },
  '处': { 'chù': ['到处', '好处', '深处', '处处', '处长', '去处', '长处', '住处', '远处'], 'chǔ': ['处理', '处于', '处罚', '查处', '处分', '处置', '相处'] },
  '曲': { 'qǔ': ['戏曲', '歌曲', '曲艺', '乐曲', '昆曲', '作曲'], 'qū': ['曲折', '曲线', '歪曲', '扭曲', '弯曲'] },
  '量': { 'liàng': ['力量', '质量', '大量', '数量', '产量', '含量', '总量', '尽量', '能量', '重量', '容量', '流量'], 'liáng': ['商量', '测量', '衡量', '丈量', '估量'] },
  '铺': { 'pù': ['店铺', '铺子', '床铺', '当铺', '铺面', '卧铺'], 'pū': ['铺设', '铺盖', '铺轨', '铺路', '铺垫', '铺张'] },
  '舍': { 'shè': ['宿舍', '校舍', '房舍', '农舍', '舍利'], 'shě': ['舍得', '舍弃', '取舍', '割舍', '施舍', '舍身'] },
  '缝': { 'fèng': ['缝隙', '裂缝', '门缝', '牙缝', '夹缝'], 'féng': ['裁缝', '缝合', '缝纫', '缝制'] },
  '着': { 'zhe': ['随着', '接着', '有着', '跟着', '沿着', '本着'], 'zhuó': ['着眼', '着手', '着力', '着重', '穿着'], 'zháo': ['着急', '着火', '睡着'] },
  '了': { 'le': ['为了', '除了', '得了', '算了', '罢了', '完了'], 'liǎo': ['了解', '明了', '了结', '末了', '了事', '了得', '了不起'] },
  '还': { 'hái': ['还有', '还是', '还要', '还在', '还好'], 'huán': ['还书', '还钱', '归还', '偿还', '返还'] },
  '行': { 'xíng': ['行动', '行为', '行走', '进行', '实行', '行李', '不行', '可行'], 'háng': ['银行', '行业', '排行', '行列', '内行', '外行'] },
  '重': { 'zhòng': ['重要', '重量', '重点', '重大', '沉重', '体重', '轻重'], 'chóng': ['重复', '重新', '重叠', '重逢', '重来'] },
  '只': { 'zhī': ['一只', '船只', '只身'], 'zhǐ': ['只有', '只是', '只要', '只能', '只是'] },
  '都': { 'dōu': ['都有', '都是', '都要', '都来'], 'dū': ['首都', '都市', '都城'] },
  '好': { 'hǎo': ['好人', '好事', '好看', '好听', '美好', '正好', '很好', '友好'], 'hào': ['爱好', '好学', '好奇', '好客'] },
  '乐': { 'lè': ['快乐', '欢乐', '乐趣', '乐观', '游乐', '可乐'], 'yuè': ['音乐', '乐曲', '乐器', '乐队', '声乐', '乐器'] },
  '长': { 'cháng': ['长短', '长度', '长期', '长江', '长久', '延长', '很长'], 'zhǎng': ['长大', '成长', '生长', '校长', '长辈', '班长', '队长'] },
  '发': { 'fā': ['发现', '发生', '出发', '发明', '发展', '发出', '发放'], 'fà': ['头发', '理发', '发型', '黑发'] },
  '教': { 'jiāo': ['教书', '教学'], 'jiào': ['教育', '教室', '教师', '教授', '教学', '教养', '教材', '教练'] },
  '弹': { 'dàn': ['导弹', '炸弹', '子弹', '弹性', '弹药', '炮弹', '枪弹', '弹头'], 'tán': ['弹琴', '弹唱', '弹簧', '弹跳'] },
  '血': { 'xuè': ['血液', '心血', '流血', '血肉', '鲜血', '热血', '献血', '血管', '血压', '血脉'], 'xiě': ['血淋淋', '流血了'] },
  '提': { 'tí': ['提高', '提问', '提前', '提出', '提醒', '提取', '提供'], 'dī': ['提防'] },
  '相': { 'xiāng': ['相同', '相信', '互相', '相处', '相对', '相关', '相当'], 'xiàng': ['相片', '照相', '长相', '真相', '面相'] },
  '应': { 'yīng': ['应该', '应当', '应得'], 'yìng': ['应用', '应对', '反应', '适应', '答应', '呼应', '对应'] },
  '当': { 'dāng': ['当时', '当然', '应当', '相当', '担当', '当前', '当地'], 'dàng': ['当真', '恰当', '上当', '当作'] },
  '中': { 'zhōng': ['中国', '中间', '中央', '中心', '其中', '空中', '集中'], 'zhòng': ['中奖', '中毒', '击中', '看中'] },
  '朝': { 'cháo': ['朝向', '朝代', '朝廷', '朝阳'], 'zhāo': ['朝阳', '朝霞', '朝夕', '今朝'] },
  '传': { 'chuán': ['传播', '传递', '传达', '传说', '传承', '流传', '宣传'], 'zhuàn': ['传记', '自传', '外传'] },
  '降': { 'jiàng': ['下降', '降落', '降低', '降雨', '降温'], 'xiáng': ['投降', '降服'] },
  '便': { 'biàn': ['方便', '顺便', '便利', '便当', '大便'], 'pián': ['便宜'] },
  '假': { 'jiǎ': ['假装', '假如', '假话', '真假', '虚假'], 'jià': ['放假', '假期', '请假', '假日'] },
  '间': { 'jiān': ['时间', '中间', '房间', '空间', '人间', '期间'], 'jiàn': ['间接', '间隔', '离间'] },
  '将': { 'jiāng': ['将来', '即将', '将军', '将要', '必将'], 'jiàng': ['将领', '大将', '将士'] },
  '圈': { 'quān': ['圆圈', '一圈', '圈子', '花圈'], 'juàn': ['猪圈', '羊圈'] },
  '切': { 'qiē': ['切开', '切菜', '切割'], 'qiè': ['一切', '亲切', '密切', '急切', '确切'] },
  '散': { 'sàn': ['散步', '散开', '分散', '解散', '扩散', '散发'], 'sǎn': ['散文', '松散', '散装', '散漫'] },
  '少': { 'shǎo': ['多少', '很少', '少量', '减少', '少数'], 'shào': ['少年', '少女', '少爷', '年少'] },
  '鲜': { 'xiān': ['新鲜', '鲜花', '鲜美', '鲜艳', '鲜明'], 'xiǎn': ['鲜为人知', '朝鲜'] },
  '要': { 'yào': ['重要', '需要', '主要', '必要', '只要', '不要', '要求'], 'yāo': ['要求', '要挟'] },
  '载': { 'zǎi': ['记载', '转载', '连载'], 'zài': ['载重', '装载', '满载', '超载'] },
  '脏': { 'zāng': ['脏话', '弄脏', '脏水'], 'zàng': ['心脏', '内脏', '肝脏'] },
  '种': { 'zhǒng': ['种子', '种类', '品种', '树种'], 'zhòng': ['种田', '种树', '种植', '耕种'] },
  '转': { 'zhuǎn': ['转变', '转学', '转身', '转移', '好转', '旋转'], 'zhuàn': ['转动', '转圈', '旋转'] },
  '藏': { 'cáng': ['收藏', '隐藏', '蕴藏', '珍藏', '藏书', '埋藏', '躲藏'], 'zàng': ['藏族', '西藏', '宝藏'] },
  '落': { 'luò': ['落下', '降落', '落叶', '落后', '角落', '日落', '落地'], 'là': ['落下', '丢三落四'], 'lào': ['落枕'] },
  '难': { 'nán': ['困难', '难过', '难题', '难忘', '为难', '艰难'], 'nàn': ['灾难', '遇难', '苦难', '难民'] },
  '宁': { 'níng': ['宁静', '安宁', '宁愿'], 'nìng': ['宁可', '宁愿', '宁死不屈'] },
  '任': { 'rèn': ['任务', '担任', '责任', '信任', '任何', '上任'], 'rén': ['任丘', '任县'] },
  '兴': { 'xīng': ['兴起', '兴旺', '兴盛', '兴奋'], 'xìng': ['高兴', '兴趣', '兴致', '扫兴', '尽兴'] },
  '削': { 'xiāo': ['削铅笔', '削皮', '削苹果'], 'xuē': ['剥削', '削弱', '削减', '瘦削'] },
  '压': { 'yā': ['压力', '压迫', '血压', '压制', '高压'], 'yà': ['压根儿'] },
  '燕': { 'yàn': ['燕子', '燕窝'], 'yān': ['燕山', '燕京'] },
  '与': { 'yǔ': ['与其', '与共', '与众不同'], 'yù': ['参与', '与会'] },
  '正': { 'zhèng': ['正确', '正在', '正好', '正式', '正常', '端正'], 'zhēng': ['正月'] },
  '骨': { 'gǔ': ['骨头', '骨骼', '骨折', '骨干', '骨肉'], 'gū': ['骨碌'] },
  '色': { 'sè': ['颜色', '红色', '彩色', '景色', '特色', '出色'], 'shǎi': ['掉色', '落色'] },
  '磨': { 'mó': ['磨刀', '折磨', '磨练', '磨炼', '磨合'], 'mò': ['磨坊', '磨盘', '推磨'] },
  '系': { 'xì': ['关系', '联系', '系统', '体系', '系列', '系数', '星系'], 'jì': ['系鞋带', '系红领巾'] },
  '解': { 'jiě': ['解决', '解释', '了解', '理解', '解开', '解答'], 'jiè': ['解送', '押解'], 'xiè': ['解数'] },
  '禁': { 'jìn': ['禁止', '禁令', '严禁', '不禁'], 'jīn': ['禁不住', '禁受'] },
  '尽': { 'jìn': ['尽力', '尽情', '尽头', '用尽', '穷尽'], 'jǐn': ['尽管', '尽快', '尽量', '尽早'] },
  '卡': { 'kǎ': ['卡片', '卡车', '卡片', '卡通'], 'qiǎ': ['卡住', '关卡', '卡壳'] },
  '看': { 'kàn': ['看见', '看书', '观看', '好看', '看到', '看法'], 'kān': ['看门', '看守', '看护'] },
  '累': { 'lèi': ['劳累', '累坏', '很累', '累了'], 'lěi': ['积累', '累计', '连累'], 'léi': ['累赘'] },
  '模': { 'mó': ['模型', '模范', '模仿', '模糊', '模式'], 'mú': ['模样', '模具', '模板'] },
  '称': { 'chēng': ['称呼', '称赞', '名称', '称为', '号称'], 'chèn': ['称心', '对称', '相称'] },
  '挑': { 'tiāo': ['挑水', '挑选', '挑食'], 'tiǎo': ['挑战', '挑衅', '挑拨'] },
  '吐': { 'tǔ': ['吐气', '谈吐', '吐字', '吞吐'], 'tù': ['呕吐', '吐血', '吐泻'] },
  '扎': { 'zhā': ['扎针', '扎根', '扎实', '扎手'], 'zā': ['扎辫子', '包扎'], 'zhá': ['挣扎'] },
  '涨': { 'zhǎng': ['涨价', '上涨', '涨潮', '高涨'], 'zhàng': ['涨大', '膨胀', '头昏脑涨'] },
  '着': { 'zhe': ['随着', '接着', '有着', '跟着', '沿着', '本着'], 'zháo': ['着急', '着火', '睡着'], 'zhuó': ['着眼', '着手', '着力', '着重'] },
  '尽': { 'jìn': ['尽力', '尽情', '尽头', '用尽', '穷尽'], 'jǐn': ['尽管', '尽快', '尽量', '尽早'] },
  '汤': { 'tāng': ['汤水', '热汤', '喝汤', '米汤'], 'shāng': ['汤汤水水'] },
  '薄': { 'báo': ['薄饼', '薄片', '薄纸'], 'bó': ['薄弱', '淡薄', '薄荷', '单薄', '薄膜', '浅薄', '微薄', '稀薄', '刻薄', '轻薄'], 'bò': ['薄荷'] },
  '露': { 'lù': ['透露', '暴露', '揭露', '披露', '流露', '显露', '泄露', '表露', '雨露', '露水'], 'lòu': ['露面', '露馅', '露马脚'] },
  '逮': { 'dǎi': ['逮住', '逮到'], 'dài': ['逮捕'] },
  '屏': { 'píng': ['屏幕', '屏风', '屏障'], 'bǐng': ['屏息', '屏气'] },
  '奔': { 'bēn': ['奔跑', '飞奔', '狂奔', '奔驰'], 'bèn': ['投奔', '奔头'] },
  '创': { 'chuàng': ['创造', '创作', '创新', '创建'], 'chuāng': ['创伤', '创口', '重创'] },
  '给': { 'gěi': ['给你', '给我', '送给'], 'jǐ': ['供给', '给予', '补给'] },
  '冠': { 'guān': ['衣冠', '树冠', '皇冠', '桂冠'], 'guàn': ['冠军', '夺冠', '桂冠', '冠名'] },
  '号': { 'hào': ['号码', '号角', '口号', '记号', '符号'], 'háo': ['号叫', '号啕大哭'] },
  '混': { 'hùn': ['混合', '混乱', '混淆', '混入'], 'hún': ['混水摸鱼', '混蛋'] },
  '卷': { 'juǎn': ['卷烟', '卷入', '席卷', '胶卷'], 'juàn': ['画卷', '答卷', '问卷', '试卷', '长卷', '案卷', '卷宗'] },
  '强': { 'qiáng': ['强大', '强壮', '加强', '坚强', '强者'], 'qiǎng': ['勉强', '强迫', '强求'], 'jiàng': ['倔强'] },
  '悄': { 'qiāo': ['悄悄', '静悄悄'], 'qiǎo': ['悄然', '悄声'] },
  '劲': { 'jìn': ['使劲', '干劲', '费劲'], 'jìng': ['劲敌', '强劲', '劲风'] },
  '据': { 'jù': ['根据', '证据', '依据', '占据'], 'jū': ['拮据'] },
  '场': { 'chǎng': ['操场', '广场', '市场', '现场', '一场雨'], 'cháng': ['场院', '打场', '赶场'] },
  '背': { 'bèi': ['背后', '背景', '背诵', '背影', '脊背'], 'bēi': ['背包', '背东西', '背负'] },
  '场': { 'chǎng': ['操场', '广场', '市场', '现场'], 'cháng': ['场院', '打场'] },
  '打': { 'dǎ': ['打人', '打球', '打开', '打扫', '打败'], 'dá': ['一打'] },
  '待': { 'dài': ['等待', '对待', '招待', '期待', '接待'], 'dāi': ['待一会儿', '待着'] },
  '恶': { 'è': ['恶劣', '凶恶', '善恶', '恶人'], 'ě': ['恶心'], 'wù': ['厌恶', '好恶'] },
  '缝': { 'féng': ['裁缝', '缝合', '缝纫', '缝制'], 'fèng': ['缝隙', '裂缝', '门缝', '牙缝', '夹缝'] },
  '服': { 'fú': ['衣服', '服务', '服从', '服装', '舒服'], 'fù': ['一服药'] },
  '供': { 'gōng': ['提供', '供应', '供给', '供水', '供电', '供求', '供热'], 'gòng': ['供养', '供奉', '供品', '上供'] },
  '观': { 'guān': ['观看', '观察', '参观', '壮观', '观点'], 'guàn': ['道观', '寺观'] },
  '龟': { 'guī': ['乌龟', '龟甲', '海龟'], 'jūn': ['龟裂'], 'qiū': ['龟兹'] },
  '横': { 'héng': ['横线', '横着', '横跨', '横向'], 'hèng': ['蛮横', '横财'] },
  '哄': { 'hōng': ['哄笑', '哄抬', '哄堂大笑'], 'hǒng': ['哄孩子', '哄骗'], 'hòng': ['起哄', '一哄而散'] },
  '监': { 'jiān': ['监督', '监考', '监视', '监管'], 'jiàn': ['太监', '监生'] },
  '拉': { 'lā': ['拉手', '拉开', '拉车', '拖拉'], 'lá': ['拉家常', '拉屎'] },
  '埋': { 'mái': ['埋藏', '埋葬', '埋伏', '埋没'], 'mán': ['埋怨'] },
  '秘': { 'mì': ['秘密', '秘书', '神秘', '奥秘'], 'bì': ['秘鲁'] },
  '抹': { 'mǒ': ['涂抹', '抹杀', '抹黑'], 'mò': ['抹墙', '抹不开'], 'mā': ['抹布', '抹桌子'] },
  '哪': { 'nǎ': ['哪里', '哪个', '哪些', '哪有'], 'na': ['天哪'] },
  '泥': { 'ní': ['泥土', '水泥', '泥沙', '泥巴'], 'nì': ['拘泥', '泥古'] },
  '奇': { 'qí': ['奇怪', '好奇', '奇迹', '奇特', '神奇'], 'jī': ['奇数'] },
  '翘': { 'qiào': ['翘起来', '翘尾巴'], 'qiáo': ['翘首'] },
  '亲': { 'qīn': ['亲人', '亲切', '母亲', '父亲', '亲自'], 'qìng': ['亲家'] },
  '丧': { 'sāng': ['丧事', '丧葬', '奔丧'], 'sàng': ['丧失', '沮丧', '丧生', '丧气', '懊丧', '颓丧'] },
  '扫': { 'sǎo': ['扫地', '打扫', '清扫', '扫除'], 'sào': ['扫帚', '扫把'] },
  '什': { 'shén': ['什么', '干什么', '为什么'], 'shí': ['什锦', '家什'] },
  '食': { 'shí': ['食物', '粮食', '饮食', '食品', '美食'], 'sì': ['食之'], 'yì': ['食言'] },
  '踏': { 'tà': ['踏步', '踏实', '践踏', '踏上'], 'tā': ['踏实'] },
  '通': { 'tōng': ['通过', '通知', '交通', '普通', '通行'], 'tòng': ['说通', '打通了一通'] },
  '委': { 'wěi': ['委托', '委员', '委屈', '委婉'], 'wēi': ['委蛇'] },
  '巷': { 'xiàng': ['小巷', '巷子', '街头巷尾'], 'hàng': ['巷道'] },
  '遗': { 'yí': ['遗产', '遗传', '遗留', '遗憾', '遗忘'], 'wèi': ['遗赠'] },
  '饮': { 'yǐn': ['饮料', '饮食', '饮水', '冷饮'], 'yìn': ['饮马'] },
  '粘': { 'zhān': ['粘贴', '粘住', '粘上'], 'nián': ['粘稠', '黏液'] },
  '挣': { 'zhèng': ['挣钱', '挣脱', '挣扎'], 'zhēng': ['挣扎'] },
  '爪': { 'zhuǎ': ['爪子'], 'zhǎo': ['爪牙', '鹰爪', '张牙舞爪'] },
  '著': { 'zhù': ['著名', '著作', '编著', '显著'], 'zhuó': ['执著'] },
  '作': { 'zuò': ['作业', '工作', '作品', '作用', '作文'], 'zuō': ['作坊', '作揖'] },
  '折': { 'zhé': ['曲折', '折磨', '挫折', '折腾', '折射', '转折', '折合', '折扣', '存折', '打折', '骨折'], 'shé': ['折本', '树枝折了'], 'zhē': ['折腾'] },
  '钻': { 'zuān': ['钻研', '钻洞', '钻探', '钻心'], 'zuàn': ['钻石', '钻头', '电钻'] },
  '率': { 'lǜ': ['效率', '概率', '频率', '速率', '汇率'], 'shuài': ['率领', '率队', '轻率', '直率'] },
  '给': { 'gěi': ['给你', '给我', '送给', '交给'], 'jǐ': ['供给', '给予'] },
  '汤': { 'tāng': ['汤水', '热汤', '喝汤', '米汤'], 'shāng': ['汤汤水水'] },
  '的': { 'de': ['我的', '你的', '他的', '好的', '是的'], 'dí': ['的确'], 'dì': ['目的', '标的'] },
  '地': { 'dì': ['土地', '大地', '地方', '地区', '地球', '地面'], 'de': ['轻轻地', '慢慢地', '飞快地'] },
  '得': { 'dé': ['得到', '获得', '取得', '懂得', '觉得'], 'de': ['跑得快', '说得好'], 'děi': ['得亏', '非得'] },
  '薄': { 'bó': ['薄弱', '淡薄', '薄荷', '单薄', '薄膜', '浅薄', '微薄', '稀薄', '刻薄', '轻薄'], 'báo': ['薄饼', '薄片', '薄纸'] },
  '绿': { 'lǜ': ['绿色', '绿叶', '绿灯', '绿化', '碧绿'], 'lù': ['绿林', '鸭绿江'] },
  '没': { 'méi': ['没有', '没事', '没空', '没来'], 'mò': ['淹没', '沉没', '埋没', '出没', '没收'] },
  '漂': { 'piāo': ['漂流', '漂浮', '漂泊'], 'piào': ['漂亮'], 'piǎo': ['漂白', '漂洗'] },
  '石': { 'shí': ['石头', '岩石', '石油', '矿石'], 'dàn': ['一石米'] },
  '识': { 'shí': ['认识', '知识', '识字', '识别'], 'zhì': ['标识', '博闻强识'] },
  '数': { 'shù': ['数字', '数学', '数量', '无数', '分数'], 'shǔ': ['数一数', '数数', '数不清'], 'shuò': ['数见不鲜'] },
  '属': { 'shǔ': ['属于', '家属', '金属', '属性'], 'zhǔ': ['属文', '属意'] },
  '似': { 'sì': ['似乎', '相似', '类似', '好似'], 'shì': ['似的'] },
  '舍': { 'shè': ['宿舍', '校舍', '房舍', '农舍'], 'shě': ['舍得', '舍弃', '取舍', '割舍', '施舍'] },
  '省': { 'shěng': ['省事', '节省', '省略', '省份', '广东省'], 'xǐng': ['省悟', '反省', '不省人事'] },
  '盛': { 'shèng': ['盛开', '盛大', '茂盛', '旺盛', '兴盛'], 'chéng': ['盛饭', '盛水'] },
  '宿': { 'sù': ['宿舍', '归宿', '住宿', '寄宿', '食宿', '宿营'], 'xiǔ': ['一宿', '整宿'], 'xiù': ['星宿'] },
  '吐': { 'tǔ': ['吐气', '谈吐', '吐字'], 'tù': ['呕吐', '吐血'] },
  '兴': { 'xīng': ['兴起', '兴旺', '兴奋'], 'xìng': ['高兴', '兴趣', '兴致', '扫兴'] },
  '压': { 'yā': ['压力', '压迫', '血压'], 'yà': ['压根儿'] },
  '咽': { 'yān': ['咽喉', '咽炎'], 'yàn': ['咽下去', '吞咽', '细嚼慢咽'], 'yè': ['呜咽', '哽咽'] },
  '要': { 'yào': ['重要', '需要', '主要', '必要'], 'yāo': ['要求', '要挟'] },
  '应': { 'yīng': ['应该', '应当'], 'yìng': ['应用', '应对', '反应', '适应', '答应'] },
  '载': { 'zǎi': ['记载', '转载'], 'zài': ['载重', '装载', '满载'] },
  '折': { 'zhé': ['曲折', '折磨', '挫折', '折射', '转折', '折合', '折扣', '打折'], 'shé': ['折本'], 'zhē': ['折腾'] },
  '正': { 'zhèng': ['正确', '正在', '正好', '正式', '正常'], 'zhēng': ['正月'] },
  '只': { 'zhī': ['一只', '船只'], 'zhǐ': ['只有', '只是', '只要', '只能'] },
  '中': { 'zhōng': ['中国', '中间', '中央', '中心', '其中'], 'zhòng': ['中奖', '中毒', '击中'] },
  '转': { 'zhuǎn': ['转变', '转学', '转身', '转移', '好转'], 'zhuàn': ['转动', '转圈'] },
  '着': { 'zhe': ['随着', '接着', '有着', '跟着'], 'zháo': ['着急', '着火', '睡着'], 'zhuó': ['着眼', '着手', '着力'] },
};

/**
 * 根据组词匹配多音字的正确读音。
 * 优先匹配第一个组词（最常用/最先展示的词），确保拼音与主要组词读音一致。
 * @param {string} hanzi
 * @param {Array<{word:string}>} words 已选出的组词
 * @returns {string|null} 匹配到的读音，无匹配返回 null
 */
function resolvePolyphone(hanzi, words) {
  const poly = POLYPHONE[hanzi];
  if (!poly || !words || words.length === 0) return null;
  // 词 → 读音 倒排
  const wordToPy = {};
  for (const [py, wordList] of Object.entries(poly)) {
    for (const w of wordList) {
      if (!wordToPy[w]) wordToPy[w] = py;
    }
  }
  // 按组词顺序，第一个命中的组词决定读音
  for (const w of words) {
    if (wordToPy[w.word]) return wordToPy[w.word];
  }
  return null;
}
